...
import { createJobSchema, updateJobSchema, updateApplicationStatusSchema, applyToJobSchema } from '../schemas/job.schema';
...
router.patch('/applications/:applicationId/withdraw', protect, restrictTo(Role.CANDIDATE), jobController.withdrawApplication);
router.patch(
...
// Candidate actions
router.post('/:id/apply', protect, restrictTo(Role.CANDIDATE), validate(applyToJobSchema), jobController.applyToJob);
router.post('/:id/save', protect, restrictTo(Role.CANDIDATE), jobController.toggleSaveJob);

// Employer actions
router.post('/:id/clone', protect, restrictTo(Role.EMPLOYER), audit('JOB_CREATE', 'JobPost'), jobController.cloneJob);
router.get('/:id/applications', protect, restrictTo(Role.EMPLOYER), jobController.getJobApplications);
router.put('/:id', protect, restrictTo(Role.EMPLOYER), validate(updateJobSchema), audit('JOB_UPDATE', 'JobPost'), jobController.updateJob);
router.patch('/:id/deactivate', protect, restrictTo(Role.EMPLOYER), audit('JOB_CLOSE', 'JobPost'), jobController.deactivateJob);
