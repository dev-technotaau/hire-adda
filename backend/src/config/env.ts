...
const envSchema = z.object({
  ...
  MFA_ENABLED: z.string().default('true').transform((val) => val === 'true'),
  ...
  // Breach Detection
  HIBP_ENABLED: z.string().default('true').transform((val) => val === 'true'),
...
export const getPasswordResetExpiryHours = (): number => parseInt(env.PASSWORD_RESET_EXPIRY_HOURS, 10);
...
export const getAuthRateLimitMaxAttempts = (): number => parseInt(env.AUTH_RATE_LIMIT_MAX_ATTEMPTS, 10);
