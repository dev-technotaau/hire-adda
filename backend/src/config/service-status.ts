...
export type ServiceStatus = 'connected' | 'disconnected' | 'disabled' | 'error' | 'not_configured' | 'ready';
...
const SERVICE_CATEGORIES = {
    ...
    cache: ['Redis', 'BullMQ Job Queue', 'SMS Queue', 'WhatsApp Queue', 'FCM Queue', 'In-App Queue', 'Web Push Queue'],
    search: ['Elasticsearch'],
    auth: ['JWT Auth', 'Passport.js', 'Google OAuth', 'LinkedIn OAuth'],
    messaging: ['Kafka', 'Firebase FCM', 'Firebase Realtime DB', 'Firebase Auth'],
    ...
    monitoring: ['Sentry', 'OpenTelemetry', 'Winston Logger', 'Google Analytics', 'Morgan HTTP Logger', 'Request ID Correlation', 'Grafana Loki'],
    security: ['Cloudflare Turnstile', 'Rate Limiting', 'CORS', 'Helmet', 'CSRF Protection', 'XSS Sanitization', 'HPP Protection', 'MFA (TOTP)', 'WebAuthn (Passkeys)', 'HIBP Breach Detection', 'Cookie Parser', 'OTP Service', 'Session Management', 'HTML Sanitization', 'WAF (Web App Firewall)', 'DDoS Protection', 'CSP (Content Security Policy)'],
    features: ['Feature Flags', 'Audit Logging', 'Swagger API Docs', 'PDF/Resume Generation', 'Excel Reports', 'Webhooks', 'AI Matching Engine', 'Resume Parsing', 'Content Moderation', 'Consent Management', 'Support Tickets', 'File Uploads', 'Identity Verification', 'GDPR Data Export', 'User Presence', 'QR Code Generation'],
...
export const displayStartupStatus = (): void => {
    ...
    // Summary
    const connected = Array.from(services.values()).filter((s) =>
        s.status === 'connected' || s.status === 'ready'
    ).length;
    const notConfigured = Array.from(services.values()).filter((s) => s.status === 'not_configured').length;
    ...
    console.log(`📈 Summary: ${connected}/${total} active | ${notConfigured} not configured | ${disabled} disabled | ${errors} errors`);
    console.log('═'.repeat(55) + '\n');
};
