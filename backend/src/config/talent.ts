...
try {
    if (env.GOOGLE_CLOUD_PROJECT_ID && env.FIREBASE_SERVICE_ACCOUNT) {
        ...
        companyClient = new CompanyServiceClient({ credentials, projectId: env.GOOGLE_CLOUD_PROJECT_ID });
 ...
 */
async function ensureTalentCompany(tenantName: string, externalId: string, displayName: string): Promise<string | null> {
