...
import { displayShutdownStatus, displayStartupStatus, logServiceShutdown, registerService } from './service-status';
...
export const initializeServices = async (): Promise<void> => {
    ...
    if (env.FIREBASE_DATABASE_URL) {
        registerService('Firebase Realtime DB', 'connected', env.FIREBASE_DATABASE_URL.replace(/https?:\/\//, '').slice(0, 40));
    } else {
    ...
    if (env.OTEL_ENABLED === 'true') {
        const endpoint = env.OTEL_EXPORTER_OTLP_ENDPOINT?.replace(/https?:\/\//, '').replace(/\/.*/, '') || 'localhost';
    ...
    if (env.LOG_AGGREGATION_URL) {
        registerService('Grafana Loki', 'connected', env.LOG_AGGREGATION_URL.replace(/https?:\/\//, '').substring(0, 30) + '...');
    } else {
    ...
    // OTP Service
    registerService('OTP Service', 'ready', `${env.OTP_LENGTH}-digit / ${env.OTP_EXPIRY_MINUTES}min expiry`);
    ...
    if (env.WEBHOOK_SECRET) {
        registerService('Webhooks', 'ready', env.WEBHOOK_CALLBACK_URL?.replace(/https?:\/\//, '').slice(0, 40));
    } else {
    ...
    if (fs.existsSync(path.join(rootDir, 'Dockerfile')) || fs.existsSync(path.join(rootDir, 'Dockerfile.dev'))) {
