...
export const updateCompanyProfileSchema = z.object({
    body: z.object({
        ...
        cinNumber: z.string().regex(/^[A-Z]\d{5}[A-Z]{2}\d{4}[A-Z]{3}\d{6}$/, 'Invalid CIN format').optional().or(z.literal('')),
        panNumber: z.string().regex(/^[A-Z]{5}\d{4}[A-Z]$/, 'Invalid PAN format').optional().or(z.literal('')),

        // Funding
        fundingStage: z.enum(['BOOTSTRAPPED', 'SEED', 'SERIES_A', 'SERIES_B', 'SERIES_C', 'SERIES_D_PLUS', 'PRE_IPO', 'PUBLIC', 'ACQUIRED', 'NOT_APPLICABLE']).optional(),
        ...
        structuredPerks: z.array(z.object({
            category: z.string().min(1),
            perks: z.array(z.string()),
        })).optional(),
        workplacePolicies: z.array(z.object({
            title: z.string().min(1),
            description: z.string().optional(),
        })).optional(),
        awardsRecognitions: z.array(z.object({
            title: z.string().min(1),
        ...
        employeeTestimonials: z.array(z.object({
            name: z.string(),
