...
export const jobApplicationReceived = (candidateName: string, jobTitle: string, companyName: string): EmailTemplate => ({
...
export const newApplicationForEmployer = (
    ...
    html: emailLayout(`
        ...
        ${infoBox([
            ...
            { label: 'Position', value: jobTitle },
            { label: 'Applied', value: new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) },
        ])}
...
export const interviewScheduled = (candidateName: string, jobTitle: string, date: string, meetLink: string): EmailTemplate => ({
...
export const jobOfferReceived = (candidateName: string, jobTitle: string, companyName: string, offerLink: string): EmailTemplate => ({
...
export const jobRejection = (candidateName: string, jobTitle: string, companyName: string): EmailTemplate => ({
...
export const applicationStatusUpdate = (
    ...
    const statusConfig: Record<string, { emoji: string; bg: string; message: string }> = {
        ...
        rejected: { emoji: '&#128220;', bg: '#f9fafb', message: 'The hiring team has decided to proceed with other candidates.' },
        withdrawn: { emoji: '&#128221;', bg: '#f9fafb', message: 'Your application has been withdrawn as requested.' },
    };

    const config = statusConfig[status.toLowerCase()] || { emoji: '&#128220;', bg: '#f9fafb', message: `Your application status has been updated to: ${status}` };
...
export const jobAlert = (candidateName: string, jobs: Array<{ title: string; company: string; location?: string; link: string }>): EmailTemplate => {
    ...
    const jobsText = jobs.map(job => `- ${job.title} at ${job.company}${job.location ? ` (${job.location})` : ''}: ${job.link}`).join('\n');
