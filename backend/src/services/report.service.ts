...
class ReportService {
    ...
    async generateUsersReport(filters?: { role?: Role; startDate?: string; endDate?: string }) {
        ...
        const users = await prisma.user.findMany({
            ...
            select: {
                ...
                isEmailVerified: true, isMobileVerified: true, isActive: true, isSuspended: true,
                mobileNumber: true, createdAt: true, lastLoginAt: true,
            },
            orderBy: { createdAt: 'desc' },
        });

        // Return structured data that can be converted to Excel/CSV
        const headers = ['ID', 'Email', 'First Name', 'Last Name', 'Role', 'Email Verified', 'Mobile', 'Mobile Verified', 'Active', 'Suspended', 'Created At', 'Last Login'];
        const rows = users.map(u => [
            u.id, u.email, u.firstName || '', u.lastName || '', u.role,
            u.isEmailVerified ? 'Yes' : 'No', u.mobileNumber || '', u.isMobileVerified ? 'Yes' : 'No',
            u.isActive ? 'Yes' : 'No', u.isSuspended ? 'Yes' : 'No',
            u.createdAt.toISOString(), u.lastLoginAt?.toISOString() || 'Never',
        ]);
    ...
    async generateAnalyticsSummary() {
        ...
        return {
            summary: {
                totalUsers, totalCandidates, totalEmployers,
                totalJobs, activeJobs, totalApplications, verifiedEmployers,
            },
     ...
     */
    async exportUsersExcel(filters?: { role?: string; startDate?: string; endDate?: string }): Promise<Buffer> {
     ...
     */
    async exportJobsExcel(filters?: { status?: string; startDate?: string; endDate?: string }): Promise<Buffer> {
        const ExcelJS = await import('exceljs');
