...
const NOTICE_RANK: Record<string, number> = {
    IMMEDIATE: 1, FIFTEEN_DAYS: 2, THIRTY_DAYS: 3, SIXTY_DAYS: 4, NINETY_DAYS: 5, MORE_THAN_NINETY_DAYS: 6,
};
...
class MatchingService {
    ...
    async findMatchingCandidates(jobId: string): Promise<CandidateMatch[]> {
        ...
        if (job.urgencyLevel === 'URGENT' || job.urgencyLevel === 'IMMEDIATE') {
            ...
            functions.push({
                filter: { terms: { noticePeriod: ['IMMEDIATE', 'FIFTEEN_DAYS', 'THIRTY_DAYS', 'SIXTY_DAYS'] } },
                weight: 5,
            });
        }
    ...
    async findMatchingJobs(userId: string): Promise<JobMatch[]> {
        ...
        // 13. Driving license match (2%)
        if (candidate.hasDrivingLicense && candidate.drivingLicenseType && candidate.drivingLicenseType !== 'NONE') {
            functions.push({
     ...
     */
    async calculateCandidateJobMatchScore(candidateId: string, jobId: string): Promise<{
        overall: number;
        dimensions: {
            skills: number;
            experience: number;
        ...
        if (!candidate || !job) {
            return {
                overall: 0,
                dimensions: {
                    skills: 0, experience: 0, salary: 0, location: 0, industry: 0,
                    workMode: 0, education: 0, noticePeriod: 0, experienceLevel: 0,
                    functionalArea: 0, jobType: 0, educationLevel: 0, drivingLicense: 0,
                },
            };
        }
        ...
        if (candidate.expectedSalaryMin && (job.salaryMin !== null || job.salaryMax !== null)) {
            ...
            const candMax = candidate.expectedSalaryMax ? Number(candidate.expectedSalaryMax) : candMin * 1.5;
        ...
        if (candidate.currentIndustry && job.industry) {
            dimensions.industry = candidate.currentIndustry.toLowerCase() === job.industry.toLowerCase() ? 1.0 : 0.0;
        }
        ...
        if (candidate.highestEducationLevel && job.educationRequired) {
            ...
            const EDUCATION_RANK: Record<string, number> = {
                'BELOW_10TH': 1, 'HIGH_SCHOOL': 2, 'INTERMEDIATE': 3, 'DIPLOMA': 4,
                'BACHELORS': 5, 'MASTERS': 6, 'DOCTORATE': 7
            };
        ...
        if (candidate.currentRole && job.functionalArea) {
            ...
            dimensions.functionalArea = candRole.includes(jobArea) || jobArea.includes(candRole) ? 1.0 : 0.0;
        }
        ...
        // 13. Driving license match (2%)
        if (job.drivingLicenseRequired && job.drivingLicenseRequired !== 'NONE' && candidate.drivingLicenseType) {
            dimensions.drivingLicense = candidate.drivingLicenseType === job.drivingLicenseRequired ? 1.0 : 0.0;
        }

        // Calculate weighted overall score
        const weights = {
            skills: 25, experience: 12, salary: 12, location: 8, industry: 7,
            workMode: 7, education: 6, noticePeriod: 5, experienceLevel: 5,
            functionalArea: 4, jobType: 4, educationLevel: 3, drivingLicense: 2,
        };
