...
export const talentMatchingService = {
    ...
    async syncJobToTalent(jobPost: {
        ...
        await tracer.startActiveSpan('cloudtalent.syncJob', async (span) => {
            ...
            try {
                ...
                if (!companyName) {
                    logger.debug(`Cloud Talent sync skipped for job ${jobPost.id}: could not resolve company`);
                ...
                const employmentTypeMap: Record<string, string> = {
                    ...
                    INTERNSHIP: 'INTERN', FREELANCE: 'CONTRACTOR',
                };
                ...
                if (jobPost.workMode) customAttributes.workMode = { stringValues: [jobPost.workMode], filterable: true };
                if (jobPost.experienceLevel) customAttributes.experienceLevel = { stringValues: [jobPost.experienceLevel], filterable: true };
                if (jobPost.industry) customAttributes.industry = { stringValues: [jobPost.industry], filterable: true };
                ...
                if (jobPost.salaryMin || jobPost.salaryMax) {
                    job.compensationInfo = {
                        entries: [{
                            ...
                            unit: 'YEARLY',
                            range: {
                                minCompensation: jobPost.salaryMin ? { currencyCode: jobPost.currency || 'INR', units: Math.round(Number(jobPost.salaryMin)) } : undefined,
                                maxCompensation: jobPost.salaryMax ? { currencyCode: jobPost.currency || 'INR', units: Math.round(Number(jobPost.salaryMax)) } : undefined,
                            },
                        }],
                    };
                }
    ...
    async _recommendJobsElastic(candidateProfile: {
        ...
        return tracer.startActiveSpan('elasticsearch.recommendJobs', async (span) => {
            ...
            try {
                ...
                if (candidateProfile.currentRole) {
                    should.push({ multi_match: { query: candidateProfile.currentRole, fields: ['title^3', 'description'], fuzziness: 'AUTO', prefix_length: 1 } });
                }
                if (candidateProfile.skills.length > 0) {
                    should.push({ terms: { skills: candidateProfile.skills, boost: 5 } });
                    should.push({ multi_match: { query: candidateProfile.skills.slice(0, 5).join(' '), fields: ['skills.text^2'], operator: 'or' } });
                }
                if (candidateProfile.currentLocation) {
                    should.push({ bool: { should: [
                        { term: { 'location.keyword': { value: candidateProfile.currentLocation, boost: 2 } } },
                ...
                if (candidateProfile.currentLocation) {
                    functions.push({ filter: { bool: { should: [
                        { term: { 'location.keyword': candidateProfile.currentLocation } },
                    ] } }, weight: 3 });
                }
                // Boost jobs matching experience range
                functions.push({ filter: { range: { experienceMin: { lte: candidateProfile.experienceYears } } }, weight: 2 });
                // Boost jobs matching preferred work mode
                if (candidateProfile.preferredWorkMode && candidateProfile.preferredWorkMode.length > 0) {
                    functions.push({ filter: { terms: { workMode: candidateProfile.preferredWorkMode } }, weight: 3 });
                }
                // Boost jobs matching preferred industries
                if (candidateProfile.preferredIndustries && candidateProfile.preferredIndustries.length > 0) {
                    functions.push({ filter: { terms: { industry: candidateProfile.preferredIndustries } }, weight: 3 });
                }
                // Boost jobs matching preferred job type
                if (candidateProfile.preferredJobType && candidateProfile.preferredJobType.length > 0) {
                    functions.push({ filter: { terms: { type: candidateProfile.preferredJobType } }, weight: 2 });
                }
                // Boost jobs matching experience level
                if (candidateProfile.experienceLevel) {
                    functions.push({ filter: { term: { experienceLevel: candidateProfile.experienceLevel } }, weight: 2 });
                }
                // Boost jobs within salary range
                if (candidateProfile.expectedSalaryMin) {
                    functions.push({ filter: { range: { salaryMax: { gte: Number(candidateProfile.expectedSalaryMin) } } }, weight: 2 });
                }

                const searchQuery = functions.length > 0
                    ? { function_score: { query: { bool: { should, filter, minimum_should_match: 1 } }, functions, score_mode: 'sum', boost_mode: 'multiply' } }
    ...
    async _recommendJobsPrisma(candidateProfile: {
        ...
        try {
            ...
            if (candidateProfile.currentRole) {
                orConditions.push({ title: { contains: candidateProfile.currentRole, mode: 'insensitive' } });
            }
            if (candidateProfile.currentLocation) {
                orConditions.push({ location: { contains: candidateProfile.currentLocation, mode: 'insensitive' } });
            }
    ...
    async _recommendCandidatesElastic(jobPost: {
        ...
        return tracer.startActiveSpan('elasticsearch.recommendCandidates', async (span) => {
            ...
            try {
                ...
                if (jobPost.title) {
                    should.push({ multi_match: { query: jobPost.title, fields: ['currentRole^3', 'headline^2'], fuzziness: 'AUTO', prefix_length: 1 } });
                }
                if (jobPost.skills.length > 0) {
                    should.push({ terms: { skills: jobPost.skills, boost: 5 } });
                    should.push({ multi_match: { query: jobPost.skills.slice(0, 5).join(' '), fields: ['skills.text^2'], operator: 'or' } });
                }
                if (jobPost.location) {
                    should.push({ bool: { should: [
                ...
                if (jobPost.experienceLevel) {
                    functions.push({ filter: { term: { experienceLevel: jobPost.experienceLevel } }, weight: 2 });
                }
                ...
                const searchQuery = functions.length > 0
                    ? { function_score: { query: { bool: { should, minimum_should_match: 1 } }, functions, score_mode: 'sum', boost_mode: 'multiply' } }
