 ...
 */
function validateExperience(exp: ParsedResumeData['experience'][0]): {
  valid: boolean;
  warnings: string[];
} {
  ...
  if (exp.startDate && exp.endDate) {
    ...
    if (years > 50) {
      warnings.push(
        `Unreasonably long duration (${Math.round(years)} years): ${exp.company || exp.role}`
      );
    }
  }
 ...
 */
function validateEducation(edu: ParsedResumeData['education'][0]): {
  valid: boolean;
  warnings: string[];
} {
  const warnings: string[] = [];
