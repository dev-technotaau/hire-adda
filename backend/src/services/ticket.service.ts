...
import { Role, TicketStatus, TicketCategory, TicketPriority, TicketSatisfaction, NotificationType } from '@prisma/client';
...
class TicketService {
    ...
    async addMessage(
        ...
        if (senderType === 'ADMIN' && !isInternal) {
            ...
            if (ticket.status === TicketStatus.AWAITING_REPLY || ticket.status === TicketStatus.RESOLVED) {
     ...
     */
    async submitSatisfaction(ticketId: string, userId: string, satisfaction: TicketSatisfaction, comment?: string) {
        const ticket = await prisma.supportTicket.findUnique({ where: { id: ticketId } });
        if (!ticket) throw new AppError('Ticket not found', 404, 'TICKET_NOT_FOUND');
    ...
    async getTicketAnalytics(startDate?: Date, endDate?: Date) {
        ...
        return {
            ...
            bySatisfaction: bySatisfaction.map(s => ({ satisfaction: s.satisfaction, count: s._count.id })),
    ...
    private async escalateNotSatisfied(ticket: { id: string; ticketNumber: string; subject: string }) {
