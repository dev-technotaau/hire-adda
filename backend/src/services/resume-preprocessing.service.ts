...
function detectActualMimeType(buffer: Buffer): string {
    ...
    if (magic.startsWith('504b0304')) return 'application/vnd.openxmlformats-officedocument.wordprocessingml.document';
...
export async function preprocessResume(
    ...
    try {
        ...
        logger.debug(`Resume preprocessed: ${originalSize} bytes, type=${actualMimeType}, hasImages=${metadata.hasImages || false}`);
