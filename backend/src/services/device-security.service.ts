 ...
 */
async function checkDevice(userId: string, fingerprint: string, ip: string, userAgent: string): Promise<void> {
    ...
    if (user) {
        notificationService.sendNewDeviceAlert(userId, user.email, user.firstName || 'there', deviceName || 'Unknown device', location || ip).catch(() => {});
    }
}
...
async function checkLocation(userId: string, ip: string): Promise<void> {
    ...
    if (lastLocation?.latitude && lastLocation?.longitude) {
        const distance = haversineDistance(lastLocation.latitude, lastLocation.longitude, geo.lat, geo.lon);
        if (distance > 500) {
            logger.warn(`Geolocation anomaly: User ${userId} logged in from ${geo.city}, ${geo.country} — ${Math.round(distance)}km from last location`);
        }
    }
}
 ...
 */
export async function postLoginChecks(userId: string, ip: string, userAgent: string, fingerprint?: string): Promise<void> {
    try {
        await Promise.all([
