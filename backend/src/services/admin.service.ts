...
export class AdminService {
    ...
    async getRecentActivity() {
        const [recentUsers, recentJobs, recentApplications] = await prisma.$transaction([
            ...
            prisma.jobPost.findMany({
                take: 5,
                orderBy: { createdAt: 'desc' },
                select: { id: true, title: true, company: { select: { companyName: true } }, createdAt: true }
            }),
    ...
    async getUsers(role?: Role, page = 1, limit = 10) {
        ...
        const [users, total] = await prisma.$transaction([
            prisma.user.findMany({
                where,
                select: { id: true, email: true, role: true, firstName: true, isEmailVerified: true, createdAt: true },
                skip,
                take: limit,
                orderBy: { createdAt: 'desc' }
            }),
    ...
    async getUserDetails(userId: string) {
        const user = await prisma.user.findUnique({
            where: { id: userId },
            select: {
                id: true, email: true, role: true, firstName: true, lastName: true, avatar: true,
                isEmailVerified: true, isMobileVerified: true, isWhatsappVerified: true,
                isActive: true, isSuspended: true, suspendedAt: true,
                mobileNumber: true, mfaEnabled: true, createdAt: true, updatedAt: true,
                lastLoginAt: true, lastLoginIp: true, loginAttempts: true, lockUntil: true,
                candidateProfile: true, companyProfile: true,
    ...
    async getDetailedAnalytics(period: 'week' | 'month' | 'quarter') {
        ...
        const [
            newRegistrations, activeUsers, newJobs, totalApplications,
            applicationsByStatus, candidateProfiles
        ] = await prisma.$transaction([
            ...
            prisma.candidateProfile.findMany({
                select: { skills: true, currentLocation: true }, take: 1000
            })
        ]);
        ...
        candidateProfiles.forEach(p => {
            ...
            if (p.currentLocation) { locationCounts[p.currentLocation] = (locationCounts[p.currentLocation] || 0) + 1; }
        });

        const topSkills = Object.entries(skillCounts).sort((a, b) => b[1] - a[1]).slice(0, 20).map(([skill, count]) => ({ skill, count }));
        const topLocations = Object.entries(locationCounts).sort((a, b) => b[1] - a[1]).slice(0, 20).map(([location, count]) => ({ location, count }));

        return {
            period, startDate: startDate.toISOString(), endDate: now.toISOString(),
            newRegistrations, activeUsers, newJobs, totalApplications,
            applicationsByStatus: Object.fromEntries(applicationsByStatus.map(s => [s.status, s._count])),
            topSkills, topLocations,
        };
    }
    ...
    async getAuditLogs(filters: {
        action?: string; entity?: string; performedBy?: string;
    ...
    async getComprehensiveStats() {
        ...
        const [
            ...
            totalJobs, activeJobs, expiredJobs,
            newJobsWeek, newJobsMonth,
            totalApplications, appsWeek,
            verificationsPending, verificationsApproved, verificationsRejected,
            activeUsersWeek,
            candidateProfiles,
        ] = await prisma.$transaction([
            prisma.user.count(),
        ...
        return {
            users: {
                total: totalUsers, candidates: totalCandidates, employers: totalEmployers, admins: totalAdmins,
                newThisWeek: newUsersWeek, newThisMonth: newUsersMonth, activeThisWeek: activeUsersWeek,
            },
            jobs: {
                total: totalJobs, active: activeJobs, expired: expiredJobs,
                newThisWeek: newJobsWeek, newThisMonth: newJobsMonth,
            },
            applications: {
                total: totalApplications, thisWeek: appsWeek,
                conversionRate: totalJobs > 0 ? +(totalApplications / totalJobs).toFixed(2) : 0,
            },
            verifications: {
                pending: verificationsPending, approved: verificationsApproved, rejected: verificationsRejected,
            },
            topSkills,
            topLocations,
            registrationTrends,
    ...
    async getDailyActiveUsers(days: number = 30) {
        ...
        for (let i = days - 1; i >= 0; i--) {
            ...
            let total = 0, candidates = 0, employers = 0;
    ...
    async getApplications(filters: {
        keyword?: string; status?: string; jobId?: string;
        ...
        const [applications, total] = await prisma.$transaction([
            prisma.jobApplication.findMany({
                ...
                include: {
                    ...
                    candidate: { select: { user: { select: { id: true, email: true, firstName: true, lastName: true } } } },
                },
                orderBy: { appliedAt: 'desc' },
                skip,
                take: limit,
        ...
        return {
            items: applications.map(a => ({
                ...
                candidateName: [a.candidate.user.firstName, a.candidate.user.lastName].filter(Boolean).join(' ') || 'Unknown',
