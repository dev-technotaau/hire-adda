...
import { CreateListInput, UpdateListInput, AddCandidatesToListInput } from '../schemas/candidate-list.schema';
 ...
 */
export const addCandidatesToList = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        if (!req.user) throw new AppError('Not authorized', 401);
        const listId = req.params.listId as string;
 ...
 */
export const removeCandidateFromList = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        if (!req.user) throw new AppError('Not authorized', 401);
        const listId = req.params.listId as string;
 ...
 * Update member notes
 */
export const updateMemberNotes = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        if (!req.user) throw new AppError('Not authorized', 401);
        const listId = req.params.listId as string;
        const candidateId = req.params.candidateId as string;
