    ...
    try {
        ...
        if (!user) {
            ...
            publishEvent(KafkaTopics.USER_REGISTERED, user.id, {
                userId: user.id, email: user.email, role: user.role, method: 'firebase',
            });
            trackEvent(getClientId(user.id), { name: 'sign_up', params: { method: 'firebase', role: user.role } }).catch(() => {});
        } else {
            // Update last login
            ...
            publishEvent(KafkaTopics.USER_LOGIN, user.id, { userId: user.id, email: user.email, method: 'firebase' });
        ...
        // Create session
        sessionService.createSession(user.id, userAgent, ipAddress).catch(err =>
            logger.error('Failed to create session for Firebase login', err)
        );
