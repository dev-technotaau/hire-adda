...
export const jobTemplateService = {
    ...
    async createTemplate(data: { name: string; description?: string; templateData: Record<string, unknown> }): Promise<ApiResponse<JobTemplate>> {
        const res = await api.post(API.JOB_TEMPLATES.CREATE, data);
        return res.data;
    },

    async updateTemplate(id: string, data: { name?: string; description?: string; templateData?: Record<string, unknown> }): Promise<ApiResponse<JobTemplate>> {
