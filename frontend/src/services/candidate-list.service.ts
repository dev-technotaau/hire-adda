...
export const candidateListService = {
    ...
    async getList(listId: string, page = 1, limit = 20): Promise<ApiResponse<CandidateList & { members: CandidateListMember[]; pagination: { page: number; limit: number; total: number; totalPages: number; hasMore: boolean } }>> {
    ...
    async addCandidates(listId: string, data: AddCandidatesInput): Promise<ApiResponse<{ message: string; totalMembers: number }>> {
        const res = await api.post(`/api/v1/candidate-lists/${listId}/candidates`, data);
        return res.data;
    },

    async removeCandidateFromList(listId: string, candidateId: string): Promise<ApiResponse<{ message: string }>> {
        const res = await api.delete(`/api/v1/candidate-lists/${listId}/candidates/${candidateId}`);
        return res.data;
    },

    async updateMemberNotes(listId: string, candidateId: string, notes: string): Promise<ApiResponse<CandidateListMember>> {
        const res = await api.patch(`/api/v1/candidate-lists/${listId}/candidates/${candidateId}/notes`, { notes });
        return res.data;
    },
};
