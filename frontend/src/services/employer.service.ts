...
export const employerService = {
        ...
        return {
            ...
            data: {
                ...
                recentApplications: (d?.recentApplications ?? []).map((app: Record<string, unknown>) => ({
                    id: app.id as string,
                    candidateName: app.candidateName as string ??
    ...
    async getSavedCandidates(page?: number, limit?: number): Promise<PaginatedResponse<CandidateProfile>> {
    ...
    async getAnalytics(filters?: { startDate?: string; endDate?: string; groupBy?: string }): Promise<ApiResponse<EmployerAnalytics>> {
