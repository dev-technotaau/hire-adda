...
/**
 * Shared login form used by /auth/login/candidate, /auth/login/employer and
 * /auth/login/vendor. The role is locked at the page level — the form
...
export type LoginFormRole = 'candidate' | 'employer' | 'vendor';
...
const REGISTER_HREF: Record<LoginFormRole, string> = {
  ...
  vendor: ROUTES.AUTH.REGISTER_VENDOR,
};
  ...
  return (
    <>
      <div className="rounded-2xl border border-[var(--border)] bg-white p-6 shadow-sm sm:p-8">
        ...
        {/* <SocialButtons mode="login" role={role === 'employer' ? 'EMPLOYER' : role === 'vendor' ? 'VENDOR' : 'CANDIDATE'} /> */}
        ...
        {/* Switch role — fallback to chooser for users on the wrong page. The
          three other roles are always reachable from here, regardless of
          which dedicated page is currently rendered. */}
        <p className="mt-2 text-center text-xs text-[var(--text-muted)]">
          {role === 'candidate' ? (
            <Link href={ROUTES.AUTH.LOGIN} className="hover:text-[var(--text)] hover:underline">
              Sign in as Employer or Vendor →
            </Link>
          ) : role === 'employer' ? (
            <Link href={ROUTES.AUTH.LOGIN} className="hover:text-[var(--text)] hover:underline">
              Sign in as Candidate or Vendor →
            </Link>
          ) : (
            <Link href={ROUTES.AUTH.LOGIN} className="hover:text-[var(--text)] hover:underline">
              Sign in as Candidate or Employer →
            </Link>
          )}
        </p>
      </div>
