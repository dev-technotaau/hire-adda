...
import {
    WORK_STATUS_LABELS, WORK_MODE_LABELS, NOTICE_PERIOD_LABELS,
    JOB_TYPE_LABELS, SALARY_CURRENCY_LABELS,
...
export default function CompareModal({ isOpen, onClose, candidates, onRemove }: CompareModalProps) {
    ...
    const salaryDisplay = (candidate: CandidateProfile) => {
        ...
        const currency = candidate.salaryCurrency ? SALARY_CURRENCY_LABELS[candidate.salaryCurrency] || candidate.salaryCurrency : 'INR';
            ...
            <div className="overflow-x-auto -mx-6 px-6">
                <table className="w-full min-w-[600px] border-collapse">
                        ...
                        <tr className="border-b border-[var(--border)]">
                            ...
                            {candidates.map((candidate) => {
                                const name = candidate.user
                                    ...
                                    : 'Anonymous';
                                return (
                                    <th key={candidate.id} className="px-3 pb-4 text-left align-top min-w-[200px]">
                    ...
                    <tbody>
                        ...
                        <CompareRow
                            label="Current Role"
                            values={candidates.map(c => c.currentRole || '—')}
                        />
                        <CompareRow
                            label="Company"
                            values={candidates.map(c => c.currentCompany || '—')}
                        />
                        <CompareRow
                            label="Location"
                            values={candidates.map(c => c.currentLocation || '—')}
                        />
                        <CompareRow
                            label="Experience"
                            values={candidates.map(c => c.experienceYears !== undefined ? `${c.experienceYears} years` : '—')}
                        />
                        <CompareRow
                        ...
                        <CompareRow
                            label="Work Status"
                            values={candidates.map(c => c.workStatus ? WORK_STATUS_LABELS[c.workStatus] : '—')}
                        />
                        <CompareRow
                            label="Notice Period"
                            values={candidates.map(c => c.noticePeriod ? NOTICE_PERIOD_LABELS[c.noticePeriod] : '—')}
                        />
                        <CompareRow
                            label="Work Mode"
                        ...
                        <CompareRow
                            ...
                            values={candidates.map(c => c.willingToRelocate ? 'Yes' : c.willingToRelocate === false ? 'No' : '—')}
                        />
                        ...
                        <CompareRow
                            ...
                            values={candidates.map(salaryDisplay)}
                        />
                        ...
                        <CompareRow
                            ...
                            values={candidates.map(c => {
                                ...
                                return `${latest.degree || ''} ${latest.field ? `in ${latest.field}` : ''}`.trim() || '—';
                            })}
                        />
                        ...
                        <CompareRow
                            label="Email"
                            values={candidates.map(c => c.user?.email || '—')}
                        />
                        <CompareRow
                            label="Phone"
                        ...
                        <CompareRow
                            ...
                            values={candidates.map(c =>
                                c.profileCompleteness !== undefined
