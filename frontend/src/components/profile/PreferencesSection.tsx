...
import {
    ...
    ROLE_CATEGORY_SUGGESTIONS, COMPANY_NAME_SUGGESTIONS,
    VISA_STATUS_OPTIONS,
...
export default function PreferencesSection({ form, updateField }: ProfileSectionProps) {
    ...
    const addTagItem = (field: 'preferredLocations' | 'preferredIndustries' | 'preferredRoleCategories' | 'blockedCompanies', val: string) => {
        const current = (form[field] || []) as string[];
        if (val.trim() && !current.includes(val.trim())) {
            updateField(field, [...current, val.trim()]);
        }
    };
    const removeTagItem = (field: 'preferredLocations' | 'preferredIndustries' | 'preferredRoleCategories' | 'blockedCompanies', val: string) => {
        updateField(field, ((form[field] || []) as string[]).filter(v => v !== val));
    };
                ...
                <div className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-3">
                        <Select
                            ...
                            onChange={(v) => updateField('preferredJobType', v as UpdateCandidateRequest['preferredJobType'])}
                            multiple
                        ...
                        <Select
                            ...
                            onChange={(v) => updateField('preferredWorkMode', v as UpdateCandidateRequest['preferredWorkMode'])}
                            multiple
                        ...
                        <Select
                            ...
                            onChange={(v) => updateField('preferredShift', v as UpdateCandidateRequest['preferredShift'])}
                    ...
                    <div>
                        <SuggestionInput
                            ...
                            onSelect={(val) => { addTagItem('preferredLocations', val); setLocationInput(''); }}
                        />
                        <div className="mt-2 flex flex-wrap gap-1.5">
                            {(form.preferredLocations || []).map(loc => (
                                <Tag key={loc} label={loc} variant="primary" onRemove={() => removeTagItem('preferredLocations', loc)} />
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        <SuggestionInput
                            ...
                            onSelect={(val) => { addTagItem('preferredIndustries', val); setIndustryInput(''); }}
                        />
                        <div className="mt-2 flex flex-wrap gap-1.5">
                            {(form.preferredIndustries || []).map(ind => (
                                <Tag key={ind} label={ind} variant="primary" onRemove={() => removeTagItem('preferredIndustries', ind)} />
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        <SuggestionInput
                            ...
                            onSelect={(val) => { addTagItem('preferredRoleCategories', val); setRoleCatInput(''); }}
                        />
                        <div className="mt-2 flex flex-wrap gap-1.5">
                            {(form.preferredRoleCategories || []).map(role => (
                                <Tag key={role} label={role} variant="primary" onRemove={() => removeTagItem('preferredRoleCategories', role)} />
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Input
                            ...
                            onChange={(e) => updateField('travelWillingnessPercent', parseInt(e.target.value) || undefined)}
                        />
                        <Select
                            label="Visa Status"
                    ...
                    <label className="flex items-center gap-3">
                        <input type="checkbox" checked={form.willingToRelocate || false} onChange={(e) => updateField('willingToRelocate', e.target.checked)} className="h-4 w-4 rounded border-[var(--border)] text-primary" />
                        <span className="text-sm text-[var(--text)]">Willing to Relocate</span>
                    </label>
                </div>
            </Card>

            <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">Documents & Status</h2>}>
                <div className="space-y-4">
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Input
                    ...
                    <div className="space-y-3">
                        ...
                        <label className="flex items-center gap-3">
                            <input type="checkbox" checked={form.isVeteran || false} onChange={(e) => updateField('isVeteran', e.target.checked)} className="h-4 w-4 rounded border-[var(--border)] text-primary" />
                            <span className="text-sm text-[var(--text)]">Veteran</span>
                        </label>
                        <div>
                            <label className="flex items-center gap-3 mb-3">
                                <input type="checkbox" checked={form.isPhysicallyChallenged || false} onChange={(e) => updateField('isPhysicallyChallenged', e.target.checked)} className="h-4 w-4 rounded border-[var(--border)] text-primary" />
                    ...
                    <div>
                        <SuggestionInput
                            ...
                            onSelect={(val) => { addTagItem('blockedCompanies', val); setBlockedCompanyInput(''); }}
                            helperText="These companies won't see your profile in search"
                        />
                        <div className="mt-2 flex flex-wrap gap-1.5">
                            {(form.blockedCompanies || []).map(c => (
                                <Tag key={c} label={c} variant="outline" onRemove={() => removeTagItem('blockedCompanies', c)} />
                            ))}
                        </div>
                    </div>
                </div>
