...
export default function InterestsSection({ form, updateField }: ProfileSectionProps) {
    ...
    const addHobby = (val: string) => {
        ...
        if (val.trim() && !current.includes(val.trim())) updateField('hobbies', [...current, val.trim()]);
    };
    const addInterest = (val: string) => {
        const current = form.interests || [];
        if (val.trim() && !current.includes(val.trim())) updateField('interests', [...current, val.trim()]);
    };

    return (
        <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">Interests & Hobbies</h2>}>
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                        {(form.hobbies || []).map(h => (
                            <Tag key={h} label={h} variant="primary" onRemove={() => updateField('hobbies', (form.hobbies || []).filter(x => x !== h))} />
                        ))}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <SuggestionInput
                        ...
                        onSelect={(val) => { addInterest(val); setInterestInput(''); }}
                    />
                    <div className="mt-2 flex flex-wrap gap-1.5">
                        {(form.interests || []).map(i => (
                            <Tag key={i} label={i} variant="primary" onRemove={() => updateField('interests', (form.interests || []).filter(x => x !== i))} />
                        ))}
                    </div>
                </div>
            </div>
