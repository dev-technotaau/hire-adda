...
export default function SkillsSection({ form, updateField }: ProfileSectionProps) {
    ...
    const removeSkillWithProficiency = (index: number) => {
        updateField('skillsWithProficiency', (form.skillsWithProficiency || []).filter((_, i) => i !== index));
    };
    ...
    const removeITSkill = (index: number) => {
        updateField('itSkills', (form.itSkills || []).filter((_, i) => i !== index));
    };
                ...
                <div className="space-y-4">
                    <div className="flex gap-2">
                        <div className="flex-1">
                            <SuggestionInput
                                ...
                                onSelect={(val) => { if (val && !form.skills?.includes(val)) { updateField('skills', [...(form.skills || []), val]); setSkillInput(''); } }}
                            />
                        </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {(form.skills || []).map((skill) => (
                            <Tag key={skill} label={skill} variant="primary" onRemove={() => removeSkill(skill)} />
                        ))}
                            ...
                            <div key={i} className="flex items-center gap-3">
                                ...
                                <div className="w-40">
                                    <Select
                                        ...
                                        onChange={(v) => updateSkillWithProficiency(i, { proficiency: v as SkillWithProficiency['proficiency'] })}
                                    />
                                </div>
                                <div className="w-24">
                                    <Input
                                        ...
                                        onChange={(e) => updateSkillWithProficiency(i, { yearsOfExperience: parseInt(e.target.value) || undefined })}
                                    />
                                </div>
                                <button onClick={() => removeSkillWithProficiency(i)} className={`text-[var(--error)] hover:text-[var(--error-dark)] ${i === 0 ? 'mt-6' : ''}`}>
                            ...
                            <div key={i} className="flex items-center gap-3">
                                ...
                                <div className="w-24">
                                    <Input
                                        ...
                                        onChange={(e) => updateITSkill(i, { experienceYears: parseInt(e.target.value) || undefined })}
                                    />
                                </div>
                                <div className="w-40">
                                    <Select
                                        ...
                                        onChange={(v) => updateITSkill(i, { proficiency: v as ITSkillEntry['proficiency'] })}
                                    />
                                </div>
                                <button onClick={() => removeITSkill(i)} className={`text-[var(--error)] hover:text-[var(--error-dark)] ${i === 0 ? 'mt-6' : ''}`}>
                                    <Trash2 className="h-4 w-4" />
                                </button>
                            </div>
                        ))
