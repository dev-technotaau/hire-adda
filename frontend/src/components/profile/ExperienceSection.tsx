...
import {
    ...
    INDUSTRY_SUGGESTIONS, DEPARTMENT_SUGGESTIONS,
            ...
            <div className="space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    ...
                    <Select
                        ...
                        onChange={(v) => updateField('noticePeriod', v as UpdateCandidateRequest['noticePeriod'])}
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    ...
                    <Input
                        ...
                        onChange={(e) => updateField('expectedSalaryMin', parseInt(e.target.value) || undefined)}
                    />
                    <Input
                        ...
                        onChange={(e) => updateField('expectedSalaryMax', parseInt(e.target.value) || undefined)}
                    />
                </div>
                ...
                {form.hasCareerBreak && (
                    <div className="grid gap-4 sm:grid-cols-2">
                        <Select
                            ...
                            onChange={(v) => updateField('careerBreakType', v as UpdateCandidateRequest['careerBreakType'])}
                ...
                {(form.experience || []).map((exp, i) => (
                    <div key={i} className="rounded-lg border border-[var(--border)] p-4 space-y-3">
                        <div className="flex items-center justify-between">
                            ...
                            <button onClick={() => removeExperience(i)} className="text-[var(--error)] hover:text-[var(--error-dark)]">
                                <Trash2 className="h-4 w-4" />
                            </button>
                        </div>
                        <div className="grid gap-3 sm:grid-cols-2">
                            <SuggestionInput label="Company" value={exp.company} onChange={(val) => updateExperience(i, { company: val })} suggestions={COMPANY_NAME_SUGGESTIONS} required />
                            <SuggestionInput label="Job Title" value={exp.role} onChange={(val) => updateExperience(i, { role: val })} suggestions={ROLE_CATEGORY_SUGGESTIONS} required />
                        </div>
                        <div className="grid gap-3 sm:grid-cols-3">
                            <SuggestionInput label="Location" value={exp.location || ''} onChange={(val) => updateExperience(i, { location: val })} suggestions={LOCATION_SUGGESTIONS} />
                            <SuggestionInput label="Industry" value={exp.industry || ''} onChange={(val) => updateExperience(i, { industry: val })} suggestions={INDUSTRY_SUGGESTIONS} />
                            <SuggestionInput label="Department" value={exp.department || ''} onChange={(val) => updateExperience(i, { department: val })} suggestions={DEPARTMENT_SUGGESTIONS} />
                        </div>
                        <div className="grid gap-3 sm:grid-cols-3">
                            <Input label="Employment Type" placeholder="e.g. Full-time" value={exp.employmentType || ''} onChange={(e) => updateExperience(i, { employmentType: e.target.value })} />
                            <DatePicker label="Start Date" mode="month" value={exp.startDate} onChange={(val) => updateExperience(i, { startDate: val })} required />
                            <DatePicker label="End Date" mode="month" value={exp.endDate || ''} onChange={(val) => updateExperience(i, { endDate: val })} disabled={exp.isCurrent} helperText={exp.isCurrent ? 'Present' : ''} />
                        </div>
                        <label className="flex items-center gap-3">
                            <input type="checkbox" checked={exp.isCurrent || false} onChange={(e) => updateExperience(i, { isCurrent: e.target.checked, endDate: e.target.checked ? undefined : exp.endDate })} className="h-4 w-4 rounded border-[var(--border)] text-primary" />
                            <span className="text-sm text-[var(--text)]">I currently work here</span>
                        </label>
                        <div className="grid gap-3 sm:grid-cols-3">
                            <Input label="Team Size" type="number" value={exp.teamSize?.toString() || ''} onChange={(e) => updateExperience(i, { teamSize: parseInt(e.target.value) || undefined })} />
                            <Input label="Reporting To" value={exp.reportingTo || ''} onChange={(e) => updateExperience(i, { reportingTo: e.target.value })} />
                            <Input label="Annual CTC" type="number" placeholder="e.g. 1200000" value={exp.annualCtc?.toString() || ''} onChange={(e) => updateExperience(i, { annualCtc: parseInt(e.target.value) || undefined })} />
                        </div>
                        <Textarea label="Description" value={exp.description || ''} onChange={(e) => updateExperience(i, { description: e.target.value })} rows={2} />
                        <Textarea label="Key Achievements" value={(exp.keyAchievements || []).join('\n')} onChange={(e) => updateExperience(i, { keyAchievements: e.target.value.split('\n').filter(Boolean) })} rows={2} helperText="One achievement per line" />
                    </div>
                ))}
            </div>
        </Card>
