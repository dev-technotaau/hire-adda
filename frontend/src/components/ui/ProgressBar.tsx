...
function ProgressBar({ value, size = 'md', color = 'primary', showLabel = false, className }: ProgressBarProps) {
        ...
        <div className={cn('w-full', className)}>
            {showLabel && (
                <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium text-[var(--text-secondary)]">{Math.round(clampedValue)}%</span>
                </div>
            )}
            <div
                className={cn(
