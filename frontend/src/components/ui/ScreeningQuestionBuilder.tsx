...
export default function ScreeningQuestionBuilder({ questions, onChange }: ScreeningQuestionBuilderProps) {
    const addQuestion = () => {
        ...
        <div>
            <div className="flex items-center justify-between mb-3">
                <label className="text-sm font-medium text-[var(--text)]">
                    Screening Questions
                    <span className="ml-2 text-xs text-[var(--text-muted)]">({questions.length}/{MAX_QUESTIONS})</span>
                </label>
                <Button
                    variant="outline"
                    size="sm"
                ...
                {questions.map((sq, idx) => (
                    <div key={idx} className="rounded-lg border border-[var(--border)] p-3 space-y-3">
                        <div className="flex items-start gap-2">
                            ...
                            <Button variant="ghost" size="sm" onClick={() => removeQuestion(idx)} className="shrink-0 mt-0.5">
                                <X className="h-4 w-4" />
                            </Button>
                        </div>
                        ...
                        <Input
                            ...
                            placeholder={sq.questionType === 'YES_NO' ? 'e.g. Yes' : sq.questionType === 'NUMERIC' ? 'e.g. 5' : 'e.g. Expected answer'}
