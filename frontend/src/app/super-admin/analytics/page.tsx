...
import {
    BarChart3, Users, UserPlus, Briefcase, FileText, TrendingUp,
    Eye, CalendarDays, Download, Code, DollarSign, Target,
    Activity, ArrowUpRight, ArrowDownRight, ShieldCheck,
    Clock, Zap, Radio, Shield, MapPin, AlertTriangle, Server,
} from 'lucide-react';
...
const STATUS_COLORS: Record<string, string> = {
    ...
    REJECTED: '#EF4444', WITHDRAWN: '#6B7280', PENDING: '#F59E0B',
    REVIEWING: '#3B82F6', INTERVIEW: '#06B6D4',
};

const STATUS_LABELS: Record<string, string> = {
    APPLIED: 'Applied', VIEWED: 'Viewed', SHORTLISTED: 'Shortlisted',
    INTERVIEW_SCHEDULED: 'Interview', OFFERED: 'Offered', HIRED: 'Hired',
    REJECTED: 'Rejected', WITHDRAWN: 'Withdrawn', PENDING: 'Pending',
    REVIEWING: 'Reviewing', INTERVIEW: 'Interview',
};

const FUNNEL_COLORS = ['#3B82F6', '#8B5CF6', '#F59E0B', '#6366F1', '#10B981', '#059669', '#EF4444', '#6B7280'];
...
export default function SuperAdminAnalyticsPage() {
    const [activeTab, setActiveTab] = useState<'overview' | 'applications' | 'advanced' | 'system'>('overview');
    ...
    const deltas = useMemo(() => {
        ...
        return {
            registrations: computeDelta(sum(secondHalf, 'registrations'), sum(firstHalf, 'registrations')),
    ...
    const userDistPieData = stats ? [
        ...
        ...(stats.totalAdmins ? [{ name: 'Admins', value: stats.totalAdmins, color: '#8B5CF6' }] : []),
    ] : [];
    ...
    const verificationTotal = (stats?.pendingVerifications ?? 0) + (stats?.verificationsApproved ?? 0) + (stats?.verificationsRejected ?? 0);
    ...
    const growthVelocity = useMemo(() => {
        ...
        const engagementRate = stats?.totalUsers ? Math.round(((stats.activeThisWeek ?? 0) / stats.totalUsers) * 100) : 0;
        const jobFillRate = stats?.totalJobs && stats.totalJobs > 0 ? Math.round((stats.activeJobs / stats.totalJobs) * 100) : 0;
        const appsPerJob = stats?.activeJobs && stats.activeJobs > 0 ? Math.round(stats.totalApplications / stats.activeJobs) : 0;
    ...
    const summaryCards = stats ? [
        { label: 'Total Users', value: stats.totalUsers, icon: Users, color: 'text-primary bg-primary-light', sparkData: dailyActiveUsers.map(d => ({ v: d.total })), sparkColor: '#2563EB', delta: deltas?.registrations },
        { label: 'Candidates', value: stats.totalCandidates, icon: Users, color: 'text-[var(--info)] bg-[var(--info-light)]', sparkData: dailyActiveUsers.map(d => ({ v: d.candidates })), sparkColor: '#3B82F6' },
        { label: 'Employers', value: stats.totalEmployers, icon: Briefcase, color: 'text-[var(--warning)] bg-[var(--warning-light)]', sparkData: dailyActiveUsers.map(d => ({ v: d.employers })), sparkColor: '#F59E0B' },
        { label: 'Active Jobs', value: stats.activeJobs, icon: Briefcase, color: 'text-[var(--success)] bg-[var(--success-light)]' },
        { label: 'Total Applications', value: stats.totalApplications, icon: FileText, color: 'text-[#8B5CF6] bg-[#EDE9FE]', sparkData: appStats?.dailyTrend?.map(d => ({ v: d.count })), sparkColor: '#8B5CF6', delta: deltas?.applications },
        { label: 'New This Week', value: stats.newUsersThisWeek, icon: UserPlus, color: 'text-primary bg-primary-light' },
        { label: 'New This Month', value: stats.newUsersThisMonth, icon: TrendingUp, color: 'text-[var(--success)] bg-[var(--success-light)]' },
        { label: 'Active This Week', value: stats.activeThisWeek ?? 0, icon: Activity, color: 'text-[var(--info)] bg-[var(--info-light)]' },
        { label: 'Pending Verifications', value: stats.pendingVerifications, icon: ShieldCheck, color: 'text-[var(--warning)] bg-[var(--warning-light)]' },
        { label: 'Apps/Job Ratio', value: stats.applicationConversionRate ?? 0, icon: Zap, color: 'text-[var(--text-muted)] bg-[var(--bg-tertiary)]' },
    ] : [];
            ...
            <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="flex flex-wrap gap-2">
                            ...
                            {(startDate || endDate) && (
                                <button
                                    ...
                                    className="rounded-lg px-3 py-1.5 text-xs font-medium text-[var(--text-muted)] hover:text-[var(--text)] transition-colors"
                                >
                                    Clear
                                </button>
                            )}
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
                            <div className="w-full sm:w-48">
                                <DatePicker label="Start Date" value={startDate} onChange={(v) => { setStartDate(v); setActivePreset(null); }} leftIcon={<CalendarDays className="h-4 w-4" />} />
                            </div>
                            <div className="w-full sm:w-48">
                                <DatePicker label="End Date" value={endDate} onChange={(v) => { setEndDate(v); setActivePreset(null); }} leftIcon={<CalendarDays className="h-4 w-4" />} />
                            </div>
                            <div className="w-full sm:w-48">
                                <Select label="Group By" options={groupByOptions} value={groupBy} onChange={(val) => setGroupBy(val as 'day' | 'week' | 'month')} />
                            </div>
                        </div>
                    </div>
                </Card>
                    ...
                    <>
                        ...
                        {statsLoading ? (
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                                {Array.from({ length: 10 }).map((_, i) => <Card key={i}><Skeleton variant="rect" height={100} /></Card>)}
                            </div>
                        ) : stats ? (
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                                    ...
                                    <Card key={card.label} padding="sm">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="text-xs text-[var(--text-muted)] truncate">{card.label}</p>
                                                <div className="flex items-center gap-1.5">
                                                    <p className="text-xl font-bold text-[var(--text)]">{formatNumber(card.value)}</p>
                                                    {card.delta !== undefined && card.delta !== null && (
                                                        <span className={`flex items-center text-xs font-medium ${card.delta >= 0 ? 'text-[var(--success)]' : 'text-[var(--error)]'}`}>
                                                            {card.delta >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                                                            {Math.abs(card.delta)}%
                                                        </span>
                                                    )}
                                                </div>
                                        ...
                                        {card.sparkData && card.sparkData.length > 1 && (
                                            <div className="mt-2">
                                                <StatsChart data={card.sparkData} dataKey="v" color={card.sparkColor} height={40} />
                                            </div>
                                        )}
                                    </Card>
                                ))}
                        ...
                        {liveCounters && Object.keys(liveCounters).length > 0 && (
                            ...
                            >
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                    {[
                                        { key: 'activeUsers', label: 'Active Users', icon: Activity, color: 'text-[var(--success)]', highlight: true },
                                        { key: 'newUsersToday', label: 'New Users Today', icon: UserPlus, color: 'text-primary' },
                                        { key: 'jobsPostedToday', label: 'Jobs Posted Today', icon: Briefcase, color: 'text-[var(--warning)]' },
                                        { key: 'applicationsToday', label: 'Applications Today', icon: FileText, color: 'text-[var(--info)]' },
                                    ].map(({ key, label, icon: Icon, color, highlight }) => (
                                        <div key={key} className={`flex items-center gap-3 rounded-lg p-4 ${highlight ? 'bg-primary-light' : 'bg-[var(--bg-secondary)]'}`}>
                                            <Icon className={`h-5 w-5 ${color}`} />
                                            <div>
                                                <p className={`text-xl font-bold ${highlight ? 'text-primary' : 'text-[var(--text)]'}`}>{liveCounters[key] ?? 0}</p>
                        ...
                        {analyticsLoading ? (
                            <div className="grid gap-6 lg:grid-cols-2">
                                {Array.from({ length: 2 }).map((_, i) => <Card key={i}><Skeleton variant="rect" height={300} /></Card>)}
                            </div>
                        ) : analytics.length > 0 ? (
                            <div className="grid gap-6 lg:grid-cols-2">
                                <Card header={
                                    ...
                                    <AreaChart
                                        ...
                                        xKey="period" yKey="registrations" yKey2="applications"
                                        color="#2563EB" color2="#10B981" height={280}
                                    />
                                </Card>
                                <Card header={
                                    <div className="flex items-center gap-2">
                            ...
                            <Card><EmptyState icon={TrendingUp} title="No trend data" description="Select a date range to view platform activity trends." /></Card>
                        )}
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <Card header={
                                ...
                                {appStatsLoading ? <Skeleton variant="rect" height={280} /> : statusPieData.length > 0 ? (
                                    <PieChart data={statusPieData} height={280} innerRadius={55} />
                                ) : (
                                    <EmptyState icon={Target} title="No application data" description="Application status distribution will appear as applications are submitted." />
                                )}
                            </Card>
                            <Card header={
                                <div className="flex items-center gap-2">
                                ...
                                {statsLoading ? <Skeleton variant="rect" height={280} /> : userDistPieData.length > 0 ? (
                                    <PieChart data={userDistPieData} height={280} innerRadius={55} />
                                ) : (
                                    <EmptyState icon={Users} title="No user data" description="User distribution will appear as users register." />
                                )}
                            </Card>
                        </div>
                            ...
                            <div className="grid gap-6 lg:grid-cols-2">
                                {stats.topSkills && stats.topSkills.length > 0 && (
                                    <Card header={
                                        ...
                                        <BarChart
                                            data={stats.topSkills.slice(0, 10).map(s => ({ skill: s.skill, count: s.count }))}
                                            xKey="skill"
                                ...
                                {stats.topLocations && stats.topLocations.length > 0 && (
                                    <Card header={
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h2 className="text-lg font-semibold text-[var(--text)]">Top Candidate Locations</h2>
                                        </div>
                                    }>
                                        <BarChart
                                            data={stats.topLocations.slice(0, 10).map(l => ({ location: l.location, count: l.count }))}
                                            xKey="location"
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            {stats?.registrationTrends && stats.registrationTrends.length > 0 && (
                                <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">Registration Trends (30 Days)</h2>}>
                                    <AreaChart
                                        data={stats.registrationTrends.map(d => ({
                                            date: new Date(d.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
                                            Registrations: d.count,
                                        }))}
                                        xKey="date" yKey="Registrations" color="#2563EB" height={280}
                                    />
                                </Card>
                            )}
                            {appStats && appStats.dailyTrend.length > 0 && (
                                <Card header={
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h2 className="text-lg font-semibold text-[var(--text)]">Daily Application Volume</h2>
                                        <span className="ml-auto text-sm font-medium text-[var(--text)]">{formatNumber(appStats.total)} total</span>
                                    </div>
                                }>
                                    <AreaChart
                                        data={appStats.dailyTrend as unknown as Record<string, unknown>[]}
                                        xKey="date" yKey="count" color="#8B5CF6" height={280}
                                    />
                                </Card>
                            )}
                        </div>
                        ...
                        {stats && verificationTotal > 0 && (
                            <Card header={
                                ...
                                <div className="grid gap-6 lg:grid-cols-2">
                                    <div className="space-y-4">
                                        {[
                                            { label: 'Pending', value: stats.pendingVerifications ?? 0, color: 'bg-[var(--warning)]' },
                                            { label: 'Approved', value: stats.verificationsApproved ?? 0, color: 'bg-[var(--success)]' },
                                            { label: 'Rejected', value: stats.verificationsRejected ?? 0, color: 'bg-[var(--error)]' },
                                        ].map(item => {
                                            const pct = verificationTotal > 0 ? Math.round((item.value / verificationTotal) * 100) : 0;
                                            return (
                                                <div key={item.label}>
                                                    <div className="flex items-center justify-between mb-1">
                                                        <span className="text-sm font-medium text-[var(--text)]">{item.label}</span>
                                                        <span className="text-sm text-[var(--text-muted)]">{item.value} ({pct}%)</span>
                                                    </div>
                                    ...
                                    <PieChart
                                        data={[
                                            ...
                                            { name: 'Approved', value: stats.verificationsApproved ?? 0, color: '#10B981' },
                                            { name: 'Rejected', value: stats.verificationsRejected ?? 0, color: '#EF4444' },
                                        ]}
                                        height={200}
                                        innerRadius={40}
                                    />
                        ...
                        <Card header={
                            ...
                            {dauLoading ? <Skeleton variant="rect" height={340} /> : dailyActiveUsers.length > 0 ? (
                                <div className="space-y-4">
                                    <div className="grid grid-cols-3 gap-4">
                                        {[
                                            { label: "Today's Total", value: dailyActiveUsers[dailyActiveUsers.length - 1]?.total ?? 0, color: 'text-[var(--text)]' },
                                            { label: 'Candidates', value: dailyActiveUsers[dailyActiveUsers.length - 1]?.candidates ?? 0, color: 'text-primary' },
                                            { label: 'Employers', value: dailyActiveUsers[dailyActiveUsers.length - 1]?.employers ?? 0, color: 'text-[var(--success)]' },
                                        ].map((item) => (
                                            <div key={item.label} className="rounded-lg bg-[var(--bg-secondary)] p-3 text-center">
                                    ...
                                    <AreaChart
                                        ...
                                        xKey="date" yKey="total" yKey2="candidates"
                                        color="#2563EB" color2="#10B981" height={280}
                                    />
                                </div>
                            ) : (
                                <EmptyState icon={Eye} title="No active user data" description="Daily active user data will appear as users interact with the platform." />
                            )}
                        </Card>
                        ...
                        {growthVelocity && (
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                                {[
                                    { label: 'WoW Growth', value: `${growthVelocity.wowGrowth > 0 ? '+' : ''}${growthVelocity.wowGrowth}%`, icon: TrendingUp, color: growthVelocity.wowGrowth >= 0 ? 'text-[var(--success)] bg-[var(--success-light)]' : 'text-[var(--error)] bg-[var(--error-light)]' },
                                    { label: 'User Engagement', value: `${growthVelocity.engagementRate}%`, icon: Activity, color: 'text-primary bg-primary-light', bar: growthVelocity.engagementRate },
                                    { label: 'Active Jobs Rate', value: `${growthVelocity.jobFillRate}%`, icon: Briefcase, color: 'text-[var(--info)] bg-[var(--info-light)]', bar: growthVelocity.jobFillRate },
                                    { label: 'Apps Per Active Job', value: growthVelocity.appsPerJob.toString(), icon: Target, color: 'text-[var(--warning)] bg-[var(--warning-light)]' },
                                    { label: 'Avg DAU (7d)', value: formatNumber(growthVelocity.avgDAU), icon: Eye, color: 'text-[var(--text-muted)] bg-[var(--bg-tertiary)]' },
                                ].map(item => (
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                {'bar' in item && item.bar !== undefined && (
                                                    <div className="mt-1 h-1 w-full rounded-full bg-[var(--bg-tertiary)]">
                                                        <div className="h-1 rounded-full bg-current transition-all" style={{ width: `${Math.min(item.bar, 100)}%` }} />
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                ...
                                {dayOfWeekData.length > 0 && (
                                    <Card header={
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h2 className="text-lg font-semibold text-[var(--text)]">Application Day-of-Week Pattern</h2>
                                        </div>
                                    }>
                                        <BarChart
                                            data={dayOfWeekData}
                                ...
                                {regDayOfWeekData.length > 0 && (
                                    <Card header={
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h2 className="text-lg font-semibold text-[var(--text)]">Registration Day-of-Week Pattern</h2>
                                        </div>
                                    }>
                                        <BarChart
                        ...
                        <Card header={
                            ...
                            {analyticsLoading ? (
                                <div className="space-y-3 p-4">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} variant="rect" height={40} />)}</div>
                            ) : analytics.length > 0 ? (
                                        ...
                                        <tbody className="divide-y divide-[var(--border)]">
                                            {analytics.map((item, idx) => (
                                                <tr key={idx} className="transition-colors hover:bg-[var(--bg-secondary)]">
                                                    ...
                                                    <td className="px-4 py-3 text-right text-[var(--text-secondary)]">{item.registrations.toLocaleString()}</td>
                                                    <td className="px-4 py-3 text-right text-[var(--text-secondary)]">{item.jobPostings.toLocaleString()}</td>
                                                    <td className="px-4 py-3 text-right text-[var(--text-secondary)]">{item.applications.toLocaleString()}</td>
                                                </tr>
                                            ))}
                                            <tr className="bg-[var(--bg-secondary)] font-semibold">
                                                ...
                                                {(['registrations', 'jobPostings', 'applications'] as const).map(key => (
                                                    <td key={key} className="px-4 py-3 text-right text-[var(--text)]">
                                                        ...
                                                        {deltas && (
                                                            <span className={`ml-2 inline-flex items-center gap-0.5 text-[10px] font-medium ${deltas[key] >= 0 ? 'text-[var(--success)]' : 'text-[var(--error)]'}`}>
                                                                {deltas[key] >= 0 ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                                                                {Math.abs(deltas[key])}%
                                                            </span>
                                                        )}
                                                    </td>
                                ...
                                <EmptyState icon={BarChart3} title="No analytics data" description="Select a date range to view period breakdown." />
                            )}
                        </Card>
                    </>
                )}
                ...
                {activeTab === 'applications' && (
                    <div className="space-y-6">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                            {[
                                { label: 'Total Applications', value: appStats?.total ?? 0, icon: FileText, color: 'text-primary bg-primary-light' },
                                { label: 'Applications This Week', value: stats?.applicationsThisWeek ?? 0, icon: TrendingUp, color: 'text-[var(--success)] bg-[var(--success-light)]' },
                                { label: 'Apps/Job Ratio', value: stats?.applicationConversionRate ?? 0, icon: Zap, color: 'text-[var(--warning)] bg-[var(--warning-light)]' },
                                { label: 'Expired Jobs', value: stats?.expiredJobs ?? 0, icon: Clock, color: 'text-[var(--error)] bg-[var(--error-light)]' },
                            ].map(kpi => (
                                <Card key={kpi.label} padding="sm">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-xl font-bold text-[var(--text)]">{(kpi.value ?? 0).toLocaleString()}</p>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">Daily Application Volume</h2>}>
                                {appStatsLoading ? <Skeleton variant="rect" height={280} /> : appStats?.dailyTrend && appStats.dailyTrend.length > 0 ? (
                                    <AreaChart
                                        data={appStats.dailyTrend.map(d => ({
                                            date: new Date(d.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }),
                                            Applications: d.count,
                                        }))}
                                        xKey="date" yKey="Applications" color="#8B5CF6" height={280}
                                    />
                                ) : (
                                    <EmptyState icon={FileText} title="No data" description="Application volume data will appear as applications are submitted." />
                                )}
                            </Card>

                            <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">Status Distribution</h2>}>
                                {appStatsLoading ? <Skeleton variant="rect" height={280} /> : statusPieData.length > 0 ? (
                                    <PieChart data={statusPieData} height={280} innerRadius={55} />
                                ) : (
                                    <EmptyState icon={Target} title="No data" description="Status distribution will appear as applications are processed." />
                                )}
                            </Card>
                        </div>
                            ...
                            <Card padding="sm">
                                <div className="flex items-center gap-3 rounded-lg border border-[var(--warning)]/30 bg-[var(--warning-light)] p-4">
                                    ...
                                    <div className="flex-1">
                                        <p className="text-sm font-semibold text-[var(--text)]">Pipeline Bottleneck Detected</p>
                                        <p className="text-sm text-[var(--text-secondary)]">
                                            Lowest conversion: <strong>{bottleneck.from}</strong> → <strong>{bottleneck.to}</strong> at <strong>{bottleneck.rate}%</strong>. Consider reviewing your {bottleneck.to.toLowerCase()} process.
                                        </p>
                                    </div>
                                </div>
                            </Card>
                        ...
                        <Card header={
                            <div className="flex items-center gap-2">
                                <TrendingUp className="h-5 w-5 text-primary" />
                                <h2 className="text-lg font-semibold text-[var(--text)]">Platform Hiring Funnel</h2>
                            </div>
                        }>
                            {funnelLoading ? <Skeleton variant="rect" height={300} /> : funnelSteps.length > 0 ? (
                                <div className="space-y-3">
                                        ...
                                        return (
                                            <div key={step.status} className="flex items-center gap-4">
                                                ...
                                                <div className="w-14 shrink-0 text-right">
                                                    <Badge variant={pct > 50 ? 'success' : pct > 20 ? 'warning' : 'neutral'} size="sm">{pct}%</Badge>
                                                </div>
                                    ...
                                    {funnelSteps.some((s: { status: string }) => s.status === 'REJECTED' || s.status === 'WITHDRAWN') && (
                                        <div className="mt-4 flex gap-4 rounded-lg bg-[var(--bg-secondary)] p-3">
                                            {funnelSteps.filter((s: { status: string }) => s.status === 'REJECTED' || s.status === 'WITHDRAWN').map((s: { status: string; count: number }) => (
                                                <div key={s.status} className="flex items-center gap-2">
                                                    <span className={`h-2.5 w-2.5 rounded-full ${s.status === 'REJECTED' ? 'bg-[var(--error)]' : 'bg-gray-400'}`} />
                                                    <span className="text-sm text-[var(--text)]">{STATUS_LABELS[s.status] || s.status}: <strong>{formatNumber(s.count)}</strong></span>
                                                </div>
                                            ))}
                                        </div>
                                    )}
                                ...
                                <EmptyState icon={TrendingUp} title="No funnel data" description="Hiring funnel data will appear when BigQuery is configured." />
                            )}
                        </Card>

                        {/* Application Funnel Bar + Status Breakdown Table */}
                        <div className="grid gap-6 lg:grid-cols-2">
                            <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">Funnel Bar Chart</h2>}>
                                {funnelLoading ? <Skeleton variant="rect" height={280} /> : applicationFunnel.length > 0 ? (
                                    <BarChart
                                    ...
                                    <EmptyState icon={BarChart3} title="No data" description="Funnel chart will appear when BigQuery is configured." />
                                )}
                            </Card>

                            <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">Status Breakdown</h2>} padding="sm">
                                {appStatsLoading ? <Skeleton variant="rect" height={280} /> : statusTableData.length > 0 ? (
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-sm">
                                            <thead>
                                                <tr className="border-b border-[var(--border)]">
                                                    <th className="px-4 py-3 text-left font-medium text-[var(--text-secondary)]">Status</th>
                                                    <th className="px-4 py-3 text-right font-medium text-[var(--text-secondary)]">Count</th>
                                                    <th className="px-4 py-3 text-right font-medium text-[var(--text-secondary)]">%</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-[var(--border)]">
                                                {statusTableData.map(row => (
                                                    <tr key={row.status} className="hover:bg-[var(--bg-secondary)] transition-colors">
                                                        <td className="px-4 py-2.5">
                                                            <div className="flex items-center gap-2">
                                                                <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: STATUS_COLORS[row.status] || '#6B7280' }} />
                                                                <span className="font-medium text-[var(--text)]">{STATUS_LABELS[row.status] || row.status}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-4 py-2.5 text-right text-[var(--text-secondary)]">{row.count.toLocaleString()}</td>
                                                        <td className="px-4 py-2.5 text-right">
                                                            <Badge variant="neutral" size="sm">{row.pct}%</Badge>
                                                        </td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                    ...
                                    <EmptyState icon={Target} title="No data" description="Status breakdown will appear as applications are processed." />
                                )}
                            </Card>
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <Card header={
                                <div className="flex items-center gap-2">
                                    <Code className="h-5 w-5 text-primary" />
                                    <h2 className="text-lg font-semibold text-[var(--text)]">Top Skills in Demand</h2>
                                    <span className="ml-auto text-xs text-[var(--text-muted)]">BigQuery-powered</span>
                                </div>
                            }>
                                {skillsLoading ? <Skeleton variant="rect" height={280} /> : popularSkills.length > 0 ? (
                                    <BarChart
                                    ...
                                    <EmptyState icon={Code} title="No skill data" description="Skill demand data will appear when BigQuery is configured." />
                                )}
                            </Card>
                            ...
                            <Card header={
                                <div className="flex items-center gap-2">
                                    ...
                                    <h2 className="text-lg font-semibold text-[var(--text)]">Application Funnel (Chart)</h2>
                                </div>
                            }>
                                {funnelLoading ? <Skeleton variant="rect" height={280} /> : applicationFunnel.length > 0 ? (
                                    <BarChart
                                    ...
                                    <EmptyState icon={FileText} title="No funnel data" description="Application funnel chart will appear when BigQuery is configured." />
                                )}
                            </Card>
                        </div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                            <Card header={
                                ...
                                {growthLoading ? <Skeleton variant="rect" height={280} /> : userGrowth.length > 0 ? (
                                    <AreaChart
                                        data={userGrowth as unknown as Record<string, unknown>[]}
                                        xKey="date" yKey="registrations" color="#2563EB" height={280}
                                    />
                                ) : (
                                    <EmptyState icon={UserPlus} title="No growth data" description="User growth data will appear when BigQuery is configured." />
                                )}
                            </Card>
                            ...
                            <Card header={
                                ...
                                {jobTrendsLoading ? <Skeleton variant="rect" height={280} /> : jobTrends.length > 0 ? (
                                    <AreaChart
                                        data={jobTrends as unknown as Record<string, unknown>[]}
                                        xKey="date" yKey="jobs_posted" color="#10B981" height={280}
                                    />
                                ) : (
                                    <EmptyState icon={Briefcase} title="No job trends data" description="Job posting trends will appear when BigQuery is configured." />
                                )}
                            </Card>
                        </div>
                        ...
                        <Card header={
                            ...
                            {salaryLoading ? <Skeleton variant="rect" height={280} /> : salaryTrends.length > 0 ? (
                                <AreaChart
                                    data={salaryTrends as unknown as Record<string, unknown>[]}
                                    xKey="month" yKey="avg_min_salary" yKey2="avg_max_salary"
                                    color="#F59E0B" color2="#EF4444" height={280}
                                />
                            ) : (
                                <EmptyState icon={DollarSign} title="No salary data" description="Salary trend data will appear when BigQuery is configured." />
                            )}
                        </Card>
                    </div>
                )}
                        ...
                        <Card
                            header={
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="ml-auto text-xs text-[var(--text-muted)]">Auto-refreshes every 60s</span>
                                </div>
                            }
                        >
                            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                                {[
                                    ...
                                    { label: 'Apps This Week', value: stats?.applicationsThisWeek ?? 0, icon: TrendingUp },
                                    { label: 'Active Users (Week)', value: stats?.activeThisWeek ?? 0, icon: Users },
                                    { label: 'Health Status', value: (healthStatus as Record<string, unknown>)?.status === 'ok' ? 'Healthy' : 'Check', icon: Activity, isText: true },
                                ].map(item => (
                                    <div key={item.label} className="flex items-center gap-3 rounded-lg bg-[var(--bg-secondary)] p-3">
                                        <item.icon className="h-5 w-5 text-[var(--text-muted)]" />
                                        <div>
                                            <p className="text-lg font-bold text-[var(--text)]">
                                                {'isText' in item ? String(item.value) : (item.value as number).toLocaleString()}
                                            </p>
                                            <p className="text-xs text-[var(--text-muted)]">{item.label}</p>
                                        </div>
                                    </div>
                                ...
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                    {Object.entries(verificationStats).map(([key, value]) => (
                                        <div key={key} className="rounded-lg bg-[var(--bg-secondary)] p-4 text-center">
                                            <p className="text-2xl font-bold text-[var(--text)]">{typeof value === 'number' ? formatNumber(value) : value}</p>
                                            <p className="mt-1 text-xs capitalize text-[var(--text-muted)]">{key.replace(/_/g, ' ')}</p>
                                        </div>
                                    ))}
                                </div>
                            </Card>
                            ...
                            <Card
                                header={
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h2 className="text-lg font-semibold text-[var(--text)]">Firestore Live Counters</h2>
                                        <span className="relative ml-2 flex h-2 w-2">
                                            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[var(--success)] opacity-75" />
                                            <span className="relative inline-flex h-2 w-2 rounded-full bg-[var(--success)]" />
                                        </span>
                            ...
                            >
                                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                                    {Object.entries(liveCounters).map(([key, value]) => (
                                        <div key={key} className="flex items-center gap-3 rounded-lg bg-[var(--bg-secondary)] p-4">
                                            <Activity className="h-5 w-5 text-[var(--text-muted)]" />
                                            <div>
                                                <p className="text-xl font-bold text-[var(--text)]">{value}</p>
                                                <p className="text-xs capitalize text-[var(--text-muted)]">{key.replace(/([A-Z])/g, ' $1').trim()}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                                ...
                                <div className="space-y-3">
                                    {activity?.users?.map(u => (
                                        <div key={`u-${u.id}`} className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="truncate text-sm font-medium text-[var(--text)]">{u.firstName || u.email}</p>
                                                <p className="text-xs text-[var(--text-muted)]">New {u.role.toLowerCase()} registered</p>
                                            </div>
                                            <span className="text-xs text-[var(--text-muted)]">{formatTimestamp(u.createdAt)}</span>
                                        </div>
                                    ))}
                                    {activity?.jobs?.map(j => (
                                        <div key={`j-${j.id}`} className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                ...
                                                <p className="text-xs text-[var(--text-muted)]">New job by {j.company?.companyName || 'Unknown'}</p>
                                            </div>
                                            <span className="text-xs text-[var(--text-muted)]">{formatTimestamp(j.createdAt)}</span>
                                        </div>
                                    ))}
                                    {activity?.applications?.map(a => (
                                        <div key={`a-${a.id}`} className="flex items-center gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <p className="truncate text-sm font-medium text-[var(--text)]">{a.job?.title || 'Job Application'}</p>
                                                <p className="text-xs text-[var(--text-muted)]">
                                                    {a.candidate?.user?.email || 'Candidate'}
                                                    <span className={`ml-1.5 inline-block rounded px-1.5 py-0.5 text-[10px] font-medium ${getStatusColor(a.status)}`}>{a.status}</span>
                                                </p>
                                            </div>
                                        </div>
                                    ))}
                                    {!activity && (
                                        <div className="flex h-40 items-center justify-center text-sm text-[var(--text-muted)]">Loading activity...</div>
                                    )}
                                    {activity && !activity.users?.length && !activity.jobs?.length && !activity.applications?.length && (
                                        <EmptyState icon={Activity} title="No recent activity" description="Recent platform activity will appear here." />
                                    )}
                                </div>
                            </Card>
                            ...
                            <Card
                                header={
                                    <div className="flex items-center gap-2">
                                        <Server className="h-5 w-5 text-[var(--text-muted)]" />
                                        <h2 className="text-lg font-semibold text-[var(--text)]">Recent Kafka Events</h2>
                                        <span className="ml-auto text-xs text-[var(--text-muted)]">Last 15 events &middot; Auto-refreshes every 15s</span>
                                    </div>
                                }
                            >
                                    ...
                                    <div className="space-y-2">
                                        {kafkaEvents.map((event, i) => (
                                            ...
                                            >
                                                ...
                                                <span className="text-xs text-[var(--text-muted)]">{formatTimestamp(event.timestamp)}</span>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <EmptyState icon={Activity} title="No recent events" description="Kafka events will appear as platform activity occurs." />
                                )}
                            </Card>
                        </div>
                        ...
                        <div className="grid gap-6 lg:grid-cols-2">
                                ...
                                {statsLoading ? (
                                    ...
                                    <div className="space-y-3">
                                        {[
                                            ...
                                            { label: 'Admin Users', value: stats.totalAdmins ?? 0, color: 'text-[#8B5CF6]' },
                                            { label: 'Total Jobs', value: stats.totalJobs, color: 'text-[var(--success)]' },
                                            { label: 'Active Jobs', value: stats.activeJobs, color: 'text-[var(--warning)]' },
                                            { label: 'Total Applications', value: stats.totalApplications, color: 'text-[var(--info)]' },
                                            { label: 'New Users Today', value: stats.newUsersToday, color: 'text-primary' },
                                            { label: 'New This Week', value: stats.newUsersThisWeek, color: 'text-[var(--success)]' },
                                            { label: 'Pending Verifications', value: stats.pendingVerifications, color: 'text-[var(--warning)]' },
                                        ].map((item) => (
                                            <div key={item.label} className="flex items-center justify-between rounded-lg bg-[var(--bg-secondary)] px-4 py-2.5">
                                                <span className="text-sm text-[var(--text-secondary)]">{item.label}</span>
                                                <span className={`font-semibold ${item.color}`}>{formatNumber(item.value)}</span>
                                            </div>
                                        ))}
                                    </div>
                                ) : null}
                            ...
                            >
                                {statsLoading ? (
                                    ...
                                    <EmptyState icon={Users} title="No user data" description="User composition will appear as users register." />
                                )}
                            </Card>
                        </div>
                    </div>
