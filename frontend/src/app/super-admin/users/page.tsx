...
export default function SuperAdminUsersPage() {
    ...
    const [createForm, setCreateForm] = useState<CreateUserRequest>({ email: '', password: '', firstName: '', lastName: '', role: 'CANDIDATE' });
    ...
    const getUserActions = (user: UserListItem) => [
        {
            label: 'View Details',
            icon: Eye,
            onClick: () => { window.location.href = ROUTES.SUPER_ADMIN.USER_DETAIL(user.id); },
        },
        ...(user.isSuspended
        ...
        <DashboardLayout requiredRole={['SUPER_ADMIN']}>
            <div className="space-y-6">
                                    ...
                                    {users.map((user) => (
                                        <tr key={user.id} className="hover:bg-[var(--bg-secondary)] transition-colors">
                                            <td className="px-4 py-3 font-medium text-[var(--text)]">
                                                <Link href={ROUTES.SUPER_ADMIN.USER_DETAIL(user.id)} className="hover:text-primary transition-colors">
                ...
                <Modal
                    ...
                    onClose={() => { setShowCreateModal(false); setCreateForm({ email: '', password: '', firstName: '', lastName: '', role: 'CANDIDATE' }); }}
                    title="Create User"
                    size="md"
                    footer={
                        <div className="flex justify-end gap-3">
                            ...
                            <Button
                                ...
                                disabled={!createForm.email || !createForm.password || !createForm.firstName || !createForm.lastName}
                            >
                                Create
                            </Button>
                        </div>
                ...
                >
                    <div className="space-y-4">
                        ...
                        <Select
                            ...
                            onChange={(val) => setCreateForm(f => ({ ...f, role: val as CreateUserRequest['role'] }))}
                        />
                    </div>
                </Modal>
                ...
                <Modal
                    ...
                    footer={
                        <div className="flex justify-end gap-3">
                            ...
                            <Button variant="destructive" onClick={() => { if (deleteTarget) deleteMutation.mutate(deleteTarget.id); }} isLoading={deleteMutation.isPending}>
                                Delete
                            </Button>
                        </div>
                    }
                    ...
                    {resetStep === 'send' ? (
                        <div className="flex items-start gap-3">
                            ...
                            <p className="text-sm text-[var(--text-secondary)]">
                                ...
                                <strong>{resetPwTarget?.email}</strong> to confirm the password reset.
                        ...
                        <div className="space-y-5">
                            <div className="rounded-lg bg-[var(--bg-secondary)] px-4 py-3">
                                <p className="text-sm text-[var(--text-secondary)]">
                                    ...
                                    <span className="font-medium text-[var(--text)]">{resetPwTarget?.email}</span>{' '}
