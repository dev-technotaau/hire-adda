...
import {
    Send, Lock, User, Calendar, Clock,
    CheckCircle, XCircle, ExternalLink, Tag,
    Shield, MessageSquare, ThumbsUp, ThumbsDown, Minus,
} from 'lucide-react';
            ...
            <div key={msg.id} className={`flex py-2 ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                <div className="max-w-[75%] space-y-1">
                    <div className={`flex items-center gap-2 ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                        <span className="text-xs font-medium text-[var(--text-secondary)]">{msg.senderName}</span>
        ...
        return (
            <div className="space-y-1">
                ...
                {ticket.satisfactionComment && (
                    <p className="text-xs text-[var(--text-muted)] italic">&quot;{ticket.satisfactionComment}&quot;</p>
                )}
            </div>
        );
    };
        ...
        return (
            <DashboardLayout requiredRole={['ADMIN', 'SUPER_ADMIN']}>
                <div className="py-20 text-center">
                    ...
                    <Link href={ROUTES.ADMIN.TICKETS} className="mt-4 inline-block text-primary hover:underline">
                        Back to tickets
                    </Link>
                </div>
            </DashboardLayout>
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <Card>
                            <div className="space-y-2">
                                ...
                                <p className="text-sm text-[var(--text)] whitespace-pre-wrap">{ticket.description}</p>
                            </div>
                        </Card>
                    ...
                    <div className="space-y-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-[var(--text)]">
                                                {[ticket.user.firstName, ticket.user.lastName].filter(Boolean).join(' ') || 'N/A'}
                                            </p>
                        ...
                        <Card>
                            <div className="space-y-3">
                                <div>
                                    <h3 className="mb-1.5 text-sm font-medium text-[var(--text-secondary)]">Priority</h3>
                                    <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${TICKET_PRIORITY_COLORS[ticket.priority]}`}>
                                        {TICKET_PRIORITY_LABELS[ticket.priority]}
                                    </span>
                                </div>
                                <div>
                                    <h3 className="mb-1.5 text-sm font-medium text-[var(--text-secondary)]">Category</h3>
                                    <div className="flex items-center gap-1.5">
                                        <Tag className="h-3.5 w-3.5 text-[var(--text-muted)]" />
                                        <span className="text-sm text-[var(--text)]">{TICKET_CATEGORY_LABELS[ticket.category]}</span>
                                    </div>
                                </div>
                            </div>
                        </Card>
                        ...
                        <Card>
                            ...
                            <div className="space-y-2 text-sm">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="text-[var(--text)]">{formatDate(ticket.createdAt, 'MMM dd, yyyy HH:mm')}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="flex items-center gap-1.5 text-[var(--text-muted)]">
                                    ...
                                    <span className="text-[var(--text)]">
                                        {ticket.firstResponseAt ? formatDate(ticket.firstResponseAt, 'MMM dd, yyyy HH:mm') : '--'}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="flex items-center gap-1.5 text-[var(--text-muted)]">
                        ...
                        {/* Satisfaction */}
                        <Card>
                            <h3 className="mb-3 text-sm font-medium text-[var(--text-secondary)]">Satisfaction</h3>
                            {renderSatisfaction()}
                        </Card>
