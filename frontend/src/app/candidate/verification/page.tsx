...
import {
    ShieldCheck, Upload, Clock, CheckCircle,
    XCircle, AlertCircle, FileText, Fingerprint, Briefcase,
} from 'lucide-react';
...
import type { VerificationRequest, VerificationType, VerificationStatus } from '@/types/verification';
...
const typeDescriptions: Record<string, { description: string; documents: string; icon: typeof Fingerprint }> = {
    IDENTITY: {
        description: 'Verify your identity with a government-issued ID such as Aadhaar card, PAN card, passport, or driving license.',
        documents: 'Accepted: Aadhaar, PAN, Passport, Driving License (PDF, JPG, PNG)',
        icon: Fingerprint,
    },
    EMPLOYMENT: {
        description: 'Verify your employment history with an offer letter, experience letter, or payslip from your employer.',
        documents: 'Accepted: Offer Letter, Experience Letter, Payslip (PDF, JPG, PNG)',
        icon: Briefcase,
    },
};

const statusConfig: Record<VerificationStatus, { label: string; variant: BadgeVariant; icon: typeof CheckCircle }> = {
        ...
        <DashboardLayout requiredRole={['CANDIDATE']}>
            <div className="space-y-6">
                <div className="flex items-center justify-between">
                    ...
                    {!showForm && (
                        <Button onClick={() => setShowForm(true)} leftIcon={<ShieldCheck className="h-4 w-4" />}>
                            New Request
                        </Button>
                    )}
                </div>
                ...
                {showForm && (
                    <Card>
                        <h2 className="text-lg font-semibold text-[var(--text)] mb-4">Submit Verification Request</h2>
                        <div className="space-y-4">
                            ...
                            {typeInfo && (
                                <div className="flex items-start gap-3 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] p-4">
                                    <typeInfo.icon className="mt-0.5 h-5 w-5 shrink-0 text-primary" />
                                    <div>
                            ...
                            <div className="flex justify-end gap-3">
                                <Button variant="outline" onClick={() => { setShowForm(false); setDocumentFile(null); }}>
                                    Cancel
                                </Button>
                                <Button
                ...
                <Card>
                    <h2 className="text-lg font-semibold text-[var(--text)] mb-4">Your Verification Requests</h2>
                    {isLoading ? (
                        ...
                        <div className="divide-y divide-[var(--border)]">
                            {verifications.map((v: VerificationRequest) => {
                                ...
                                return (
                                    <div key={v.id} className="flex items-start justify-between gap-4 py-4 first:pt-0 last:pb-0">
                                            ...
                                            <div>
                                                ...
                                                {v.adminComments && (
                                                    <div className="mt-2 rounded-md bg-[var(--bg-secondary)] p-2.5 text-sm text-[var(--text-secondary)]">
                                                        <span className="font-medium">Reviewer feedback: </span>{v.adminComments}
                                                    </div>
                                                )}
                                                {v.documentUrl && (
                                                    <a
