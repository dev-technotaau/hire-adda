...
import {
    ...
    LayoutList, LayoutGrid, Users, Navigation, Map as MapIcon,
    Flame, Zap, X, ChevronDown, ChevronUp,
    ExternalLink, Send, GitCompareArrows,
} from 'lucide-react';
...
import AdvancedFilters, { ActiveFilterTags, type FilterSection, type FacetBucket } from '@/components/ui/AdvancedFilters';
...
import {
    ...
    EDUCATION_LEVEL_LABELS, FUNCTIONAL_AREA_LABELS,
...
const MapView = dynamic(() => import('@/components/jobs/MapView'), {
    ssr: false,
    loading: () => <div className="flex h-full items-center justify-center bg-[var(--bg-secondary)] rounded-xl"><Skeleton className="h-full w-full" /></div>,
});
...
function buildFilterSections(facets: SearchFacets): FilterSection[] {
    return [
        {
            key: 'workMode', label: 'Work Mode', type: 'multiselect' as const,
            options: WORK_MODE_LABELS,
            facets: facets.workMode,
            collapsible: true, defaultOpen: true,
        },
        {
            key: 'type', label: 'Job Type', type: 'multiselect' as const,
            options: JOB_TYPE_LABELS,
            facets: facets.type,
            collapsible: true, defaultOpen: true,
        },
        {
            key: 'experienceLevel', label: 'Experience Level', type: 'multiselect' as const,
        ...
        ...(facets.salaryRange?.length ? [{
            key: 'salaryBucket', label: 'Salary Range', type: 'radio' as const,
            options: Object.fromEntries(facets.salaryRange.map(b => [b.key, b.key])),
            facets: facets.salaryRange,
            collapsible: true, defaultOpen: true,
        }] : [{
            key: 'salary', label: 'Salary Range', type: 'range' as const,
        ...
        ...(facets.experienceRange?.length ? [{
            key: 'experienceBucket', label: 'Experience', type: 'radio' as const,
            options: Object.fromEntries(facets.experienceRange.map(b => [b.key, b.key])),
            facets: facets.experienceRange,
            collapsible: true, defaultOpen: false,
        }] : []),
        {
            key: 'companyType', label: 'Company Type', type: 'multiselect' as const,
        ...
        ...(facets.industry?.length ? [{
            key: 'industry', label: 'Industry', type: 'multiselect' as const,
        ...
        ...(facets.topLocations?.length ? [{
            key: 'locationFacet', label: 'Location', type: 'radio' as const,
            options: facetToOptions(facets.topLocations),
            facets: facets.topLocations,
            collapsible: true, defaultOpen: false,
        }] : []),
        {
            key: 'educationRequired', label: 'Education', type: 'multiselect' as const,
            options: EDUCATION_LEVEL_LABELS,
            facets: facets.educationRequired,
            collapsible: true, defaultOpen: false,
        },
        {
            key: 'shiftType', label: 'Shift Type', type: 'multiselect' as const,
            options: SHIFT_TYPE_LABELS,
            facets: facets.shiftType,
            collapsible: true, defaultOpen: false,
        },
        {
            key: 'urgencyLevel', label: 'Urgency', type: 'select' as const,
            options: URGENCY_LEVEL_LABELS,
            facets: facets.urgencyLevel,
            collapsible: true, defaultOpen: false,
        },
        {
            key: 'functionalArea', label: 'Functional Area', type: 'multiselect' as const,
...
const FLAT_FILTER_SECTIONS: FilterSection[] = [
    ...
    { key: 'experienceLevel', label: 'Experience Level', type: 'multiselect', options: EXPERIENCE_LEVEL_LABELS },
    { key: 'salaryBucket', label: 'Salary Range', type: 'radio', options: Object.fromEntries(Object.keys(SALARY_BUCKET_MAP).map(k => [k, k])) },
    { key: 'salary', label: 'Salary Range', type: 'range', rangePrefix: '₹' },
    { key: 'experienceBucket', label: 'Experience', type: 'radio', options: Object.fromEntries(Object.keys(EXPERIENCE_BUCKET_MAP).map(k => [k, k])) },
    ...
    { key: 'educationRequired', label: 'Education', type: 'multiselect', options: EDUCATION_LEVEL_LABELS },
    ...
    { key: 'functionalArea', label: 'Functional Area', type: 'multiselect', options: FUNCTIONAL_AREA_LABELS },
];

/* ───────────────────── URL state sync helpers ─────────────────────── */

const FILTER_URL_KEYS = [
    'keyword', 'location', 'type', 'workMode', 'experienceLevel', 'shiftType',
    'companyType', 'urgencyLevel', 'educationRequired', 'functionalArea',
    'salaryMin', 'salaryMax', 'industry', 'department',
    'isFeatured', 'isWalkIn', 'isPwdFriendly', 'visaSponsorshipAvailable',
...
function filtersToSearchParams(filters: JobSearchFilters, postedDays: string): URLSearchParams {
    ...
    for (const key of FILTER_URL_KEYS) {
        ...
        if (val && val !== '' && !(key === 'page' && val === '1') && !(key === 'sortBy' && val === 'relevance')) {
            params.set(key, val);
        }
    }
...
export default function JobSearchPage() {
    ...
    // ── State ──
    const [filters, setFilters] = useState<JobSearchFilters>(() => filtersFromSearchParams(searchParams));
    ...
    const recommendedJobs: Job[] = showRecommendations
        ? (recommendedData?.data?.items || []).filter((rj: Job) => !jobs.some(j => j.id === rj.id)).slice(0, 5)
        : [];
    ...
    const activeFilterCount = Object.entries(filters).filter(
        ([key, val]) => val && !['page', 'limit', 'sortBy', 'keyword', 'location', 'postedAfter', 'postedBefore'].includes(key)
    ...
    const filterValues = useMemo(() => {
        ...
        if (filters.experience) {
            experienceBucket = Object.entries(EXPERIENCE_BUCKET_MAP).find(
                ([, v]) => {
    ...
    const compareJobs = useMemo(() =>
        compareIds.map(id => jobs.find(j => j.id === id) || recommendedJobs.find(j => j.id === id)).filter(Boolean) as Job[],
    ...
    const handleFilterChange = useCallback((key: string, value: string | undefined) => {
        ...
        if (key === 'experienceBucket') {
            ...
            setFilters(prev => ({
                ...prev,
                experience: bucket ? (bucket.max ? `${bucket.min}-${bucket.max}` : `${bucket.min}+`) : undefined,
                page: '1',
            }));
            return;
        }
                    ...
                    <div className="min-w-0 flex-1 space-y-4">
                        ...
                        {viewMode === 'map' && (
                            <div className="space-y-3">
                                ...
                                <div className="hidden lg:flex gap-4 h-[calc(100vh-320px)] min-h-[400px]">
                                    <div className="w-[340px] shrink-0 overflow-y-auto space-y-2 pr-1">
                                        ...
                                        {jobs.length === 0 && !isLoading && (
                                            <p className="py-8 text-center text-sm text-[var(--text-muted)]">No jobs with location data</p>
                                        )}
                                    </div>
                                ...
                                <div className="lg:hidden relative h-[calc(100vh-260px)] min-h-[350px] rounded-xl overflow-hidden">
                                    <MapView
                                        jobs={jobs}
                                        selectedJobId={selectedJobId}
                                        savedJobIds={savedJobIds}
                                        ...
                                        userLng={userLng}
                                    />
                                    <div className="absolute bottom-0 left-0 right-0 max-h-[40vh] overflow-y-auto bg-white rounded-t-2xl shadow-xl p-3 space-y-2">
...
function JobCard({
    ...
    const distanceKm = userLat && userLng && job.latitude && job.longitude
        ? haversineKm(userLat, userLng, job.latitude, job.longitude)
        : undefined;
    const isNew = isPostedWithin(job.createdAt, 24);
    const isHot = (job.urgencyLevel === 'URGENT' || job.urgencyLevel === 'IMMEDIATE') && isPostedWithin(job.createdAt, 72);
    const canQuickApply = job.applyMethod === 'IN_PLATFORM' && (!job.screeningQuestions || job.screeningQuestions.length === 0);

    const skillMatchCount = candidateSkills?.size && job.skillsRequired?.length
        ? job.skillsRequired.filter(s => candidateSkills.has(s.toLowerCase())).length
        : 0;
    const skillMatchPct = job.skillsRequired?.length ? Math.round((skillMatchCount / job.skillsRequired.length) * 100) : 0;
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[var(--text-muted)]">
                            <span className="flex items-center gap-1">
                                ...
                                {job.additionalLocations.length > 0 && <span>+{job.additionalLocations.length}</span>}
                            </span>
                            <span className="flex items-center gap-1">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                            ...
                            {job.functionalArea && <Badge variant="neutral" size="sm">{FUNCTIONAL_AREA_LABELS[job.functionalArea]}</Badge>}
                        </div>
                        ...
                        {(job.skillsRequired?.length ?? 0) > 0 && (
                            <div className="mt-2 flex flex-wrap items-center gap-1">
                                ...
                                {job.skillsRequired.slice(0, 6).map((skill) => {
                                    ...
                                    const isKeywordMatch = !isProfileMatch && searchKeyword && searchKeyword.toLowerCase().split(/[\s,]+/).some(t => t.length > 1 && skill.toLowerCase().includes(t));
                                    return (
                                        <Tag key={skill} label={skill} size="sm" variant={isProfileMatch || isKeywordMatch ? 'success' : 'primary'} />
                                    );
                                })}
...
function CompactJobCard({
    ...
    return (
        <div className="group flex items-center gap-3 rounded-xl border border-[var(--border)] bg-white px-4 py-3 transition-all hover:border-primary/20 hover:shadow-sm">
            {/* Logo */}
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[var(--bg-tertiary)]">
                {job.isConfidential ? (
                    ...
                    <img src={job.company.logo} alt={job.company.companyName} className="h-8 w-8 rounded-md object-contain" />
                ) : (
            ...
            <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                    <Link
                        href={ROUTES.CANDIDATE.JOB_DETAIL(job.id)}
                        className="truncate text-sm font-semibold text-[var(--text)] hover:text-primary transition-colors"
                    >
                        <HighlightText text={job.title} highlight={searchKeyword} />
                    </Link>
                    {isNew && (
                ...
                {(job.skillsRequired?.length ?? 0) > 0 && (
                    <div className="mt-1 flex flex-wrap items-center gap-1">
                        ...
                        {job.skillsRequired.length > 4 && (
                            <span className="text-[10px] text-[var(--text-muted)]">+{job.skillsRequired.length - 4}</span>
                        )}
                    </div>
                )}
            </div>
