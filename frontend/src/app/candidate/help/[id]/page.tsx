...
import {
    ArrowLeft, Send, Lock, RotateCcw, ThumbsUp, Minus, ThumbsDown, User, Shield, Bot,
} from 'lucide-react';
            ...
            <div className="space-y-6">
                ...
                <div>
                    ...
                    <button
                        type="button"
                        onClick={() => router.push(ROUTES.CANDIDATE.HELP)}
                        className="mb-4 inline-flex items-center gap-1.5 text-sm text-[var(--text-muted)] hover:text-primary transition-colors"
                    >
    ...
    if (isSystem) {
        return (
            <div className="flex justify-center">
                <p className="text-xs italic text-[var(--text-muted)] bg-[var(--bg-secondary)] rounded-full px-4 py-1.5">
                    {message.body}
                </p>
            </div>
        );
            ...
            <div
                className={`max-w-[75%] rounded-xl px-4 py-3 ${
                    isUser
                        ...
                        : 'bg-[var(--bg-secondary)] rounded-bl-sm'
                }`}
            >
                {/* Sender info */}
                ...
                {/* Timestamp */}
                <p className={`text-[10px] text-[var(--text-muted)] mt-1.5 ${isUser ? 'text-right' : 'text-left'}`}>
