...
import {
    ...
    FileText, MessageSquare, User, Mail,
} from 'lucide-react';
...
export default function EmployerApplicationDetailPage() {
    ...
    const handleStatusUpdate = (status: string) => {
        ...
        updateStatus.mutate(
            ...
            {
                onSuccess: () => {
                    showToast.success(`Application ${APPLICATION_STATUS_LABELS[status]?.toLowerCase() || 'updated'}`);
        ...
        <DashboardLayout requiredRole={['EMPLOYER']}>
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-3">
                    <Link href={ROUTES.EMPLOYER.APPLICATIONS} className="rounded-lg p-2 hover:bg-[var(--bg-secondary)]">
                                                ...
                                                <div className="flex items-start justify-between gap-2">
                                                    ...
                                                    <div className="flex gap-1.5">
                                                        ...
                                                        {sa.question.isDealBreaker && (
                                                            <Badge variant="error" size="sm">Deal Breaker</Badge>
                                                        )}
                                                    </div>
                                                </div>
                                        ...
                                        <table className="w-full text-sm">
                                            <thead>
                                                <tr className="border-b border-[var(--border)] text-left text-[var(--text-muted)]">
                                                    ...
                                                    <th className="pb-2 pr-4 font-medium">Interviewer</th>
                                            ...
                                            <tbody>
                                                {app.interviewFeedback.map((fb, idx) => (
                                                    <tr key={idx} className="border-b border-[var(--border)] last:border-0">
                                                        ...
                                                        <td className="py-3 pr-4 text-[var(--text-light)]">{fb.interviewer || '-'}</td>
                        ...
                        {app.matchScore != null && (
                            <Card>
                                <div className="flex items-center gap-3 p-6">
                                    ...
                                    <div>
                                        <p className="text-lg font-semibold text-[var(--text)]">{Math.round(app.matchScore)}%</p>
            ...
            <Modal
                isOpen={rejectTarget}
                onClose={() => { setRejectTarget(false); setRejectionReason(''); }}
            ...
            >
                <div className="space-y-4">
                    ...
                    <div className="flex justify-end gap-3">
                        <Button variant="outline" onClick={() => { setRejectTarget(false); setRejectionReason(''); }}>
                            Cancel
                        </Button>
                        <Button
                            variant="destructive"
