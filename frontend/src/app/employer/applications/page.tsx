...
import {
    Users, Briefcase, Search, Clock, UserCheck,
...
export default function EmployerApplicationsDashboard() {
    ...
    const { data, isLoading } = useQuery({
        ...
        queryFn: () => employerService.getAllApplications({
    ...
    const updateStatusMutation = useMutation({
        mutationFn: ({ applicationId, status, rejectionReason: reason }: { applicationId: string; status: string; rejectionReason?: string }) =>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <form onSubmit={handleSearch} className="relative flex-1 max-w-md">
                        ...
                        <input
                            ...
                            className="w-full rounded-lg border border-[var(--border)] bg-[var(--bg)] py-2 pl-10 pr-3 text-sm text-[var(--text)] placeholder-[var(--text-muted)] focus:border-primary focus:outline-none"
                        />
                    </form>
                ...
                <div className="space-y-3">
                    {isLoading ? (
                        ...
                        applications.map((app: JobApplication) => {
                            ...
                            return (
                                <Card key={app.id} className="hover:border-primary/20 hover:shadow-sm transition-all">
                                    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                                        <div className="flex gap-3 min-w-0 flex-1">
                                            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary-light">
                                                {candidate?.user?.avatar ? (
                                                    <img src={candidate.user.avatar} alt={name} className="h-10 w-10 rounded-full object-cover" />
                                                ) : (
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-[var(--text-muted)]">
                                                    ...
                                                    <span className="flex items-center gap-1">
                                                        <Clock className="h-3.5 w-3.5" /> Applied {formatRelativeDate(app.appliedAt)}
                                                    </span>
                                                </div>
                                                {candidate?.skills && candidate.skills.length > 0 && (
                                                    <div className="mt-2 flex flex-wrap gap-1">
                                                        {candidate.skills.slice(0, 4).map(skill => (
                                                            <span key={skill} className="rounded bg-[var(--bg-tertiary)] px-1.5 py-0.5 text-[10px] text-[var(--text-secondary)]">
                                                                {skill}
                                                            </span>
                                                        ))}
                                                        {candidate.skills.length > 4 && (
                                                            <span className="text-[10px] text-[var(--text-muted)]">+{candidate.skills.length - 4}</span>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                                                    ...
                                                    <Button
                                                        ...
                                                        onClick={() => {
                                                            if (action.status === 'REJECTED') {
                                                                ...
                                                                updateStatusMutation.mutate({ applicationId: app.id, status: action.status });
                                                            }
                                                        }}
