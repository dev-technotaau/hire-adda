...
import {
    ArrowLeft, Send, Lock, Unlock,
    ThumbsUp, Minus, ThumbsDown, MessageSquare,
    Clock,
} from 'lucide-react';
...
export default function EmployerTicketDetailPage() {
    ...
    const satisfactionMutation = useMutation({
        mutationFn: ({ satisfaction, comment }: { satisfaction: TicketSatisfaction; comment?: string }) =>
            ...
            <DashboardLayout requiredRole={['EMPLOYER']}>
                <div className="space-y-4">
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 text-sm text-[var(--text-muted)] hover:text-primary transition-colors"
                    >
                            ...
                            {showSatisfaction ? (
                                <div className="space-y-4 max-w-md mx-auto">
                                    <textarea
                                        ...
                                        rows={3}
                                        className="w-full rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)] focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 resize-vertical"
                                    />
                                    <div className="flex items-center justify-center gap-3">
                                        <Button
                                            ...
                                            className="text-[var(--success)] border-[var(--success)]/30 hover:bg-[var(--success-light)]"
                                        >
                                            Satisfied
                                        </Button>
                                        <Button
                                            ...
                                            isLoading={satisfactionMutation.isPending}
                                            className="text-[var(--warning)] border-[var(--warning)]/30 hover:bg-[var(--warning-light)]"
                                        >
                                            Neutral
                                        </Button>
                                        <Button
                                            ...
                                            isLoading={satisfactionMutation.isPending}
                                            className="text-[var(--error)] border-[var(--error)]/30 hover:bg-[var(--error-light)]"
                                        >
                                            Not Satisfied
                                        </Button>
                                    </div>
                    ...
                    <Card className="border-[var(--success)]/30">
                        <div className="flex items-center gap-3">
                            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[var(--success-light)]">
                                {ticket.satisfaction === 'SATISFIED' && <ThumbsUp className="h-5 w-5 text-[var(--success)]" />}
                                {ticket.satisfaction === 'NEUTRAL' && <Minus className="h-5 w-5 text-[var(--warning)]" />}
                                {ticket.satisfaction === 'NOT_SATISFIED' && <ThumbsDown className="h-5 w-5 text-[var(--error)]" />}
                            </div>
                            <div>
                                <p className="text-sm font-medium text-[var(--text)]">
                                    You rated this experience as{' '}
                            ...
                            if (isSystem) {
                                return (
                                    <div key={message.id} className="flex justify-center">
                                        <div className="rounded-full bg-[var(--bg-secondary)] px-4 py-1.5 text-xs text-[var(--text-muted)] text-center max-w-md">
                                            {message.body}
                                    ...
                                    <div
                                        className={`max-w-[75%] rounded-xl px-4 py-3 ${
                                            isUserMessage
                                                ...
                                                : 'bg-[var(--bg-secondary)] text-[var(--text)] rounded-bl-sm'
                                        }`}
                                    >
