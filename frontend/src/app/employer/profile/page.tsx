...
type Section = 'company' | 'about' | 'culture' | 'benefits' | 'tech' | 'people' | 'awards' | 'legal' | 'contact' | 'address' | 'social';
...
export default function CompanyProfilePage() {
    ...
    useEffect(() => {
        if (company) {
            setForm({
                ...
                socialLinks: company.socialLinks || { linkedin: '', twitter: '', facebook: '', instagram: '', youtube: '', glassdoor: '' },
    ...
    const updateField = <K extends keyof UpdateCompanyRequest>(key: K, value: UpdateCompanyRequest[K]) => {
    ...
    const addToArray = (key: ArrayKey, value: string, clearFn: (v: string) => void) => {
        ...
        if (trimmed && !((form[key] as string[] | undefined) || []).includes(trimmed)) {
            updateField(key, [...((form[key] as string[] | undefined) || []), trimmed] as UpdateCompanyRequest[typeof key]);
            clearFn('');
        }
    };

    const removeFromArray = (key: ArrayKey, value: string) => {
        updateField(key, ((form[key] as string[] | undefined) || []).filter(v => v !== value) as UpdateCompanyRequest[typeof key]);
    };
    ...
    const renderSection = () => {
        switch (activeSection) {
            case 'company': return <CompanyInfoSection {...sectionProps} />;
            case 'about': return <AboutSection form={form} updateField={updateField} />;
            case 'culture': return <CultureSection {...sectionProps} />;
            case 'benefits': return <BenefitsSection {...sectionProps} />;
            case 'tech': return <TechPoliciesSection {...sectionProps} />;
            case 'people': return <PeopleSection form={form} updateField={updateField} />;
            case 'awards': return <AwardsFundingSection {...sectionProps} />;
            case 'legal': return <LegalSection form={form} updateField={updateField} />;
            case 'contact': return <ContactSection form={form} updateField={updateField} />;
            case 'address': return <AddressSection {...sectionProps} />;
            case 'social': return <SocialSection form={form} updateField={updateField} />;
        }
    };
            ...
            <div className="space-y-6">
                ...
                <Card padding="sm">
                    <div className="flex items-center gap-4">
                        <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-xl bg-[var(--bg-tertiary)] overflow-hidden">
                            {company?.logo ? (
                                <img src={company.logo} alt={company?.companyName || 'Company logo'} className="h-full w-full object-contain" />
                            ) : (
                                <Building2 className="h-10 w-10 text-[var(--text-muted)]" />
                            )}
                        </div>
                        <div className="flex-1">
                            <p className="font-medium text-[var(--text)]">{company?.companyName || 'Your Company'}</p>
                            <p className="text-sm text-[var(--text-muted)]">Upload a company logo (PNG, JPG, max 5MB)</p>
                            <div className="mt-2 flex gap-2">
                                <label className="cursor-pointer">
                                    ...
                                    />
                                    <span className="inline-flex items-center rounded-lg border border-[var(--border)] px-3 py-1.5 text-sm font-medium text-[var(--text)] hover:bg-[var(--bg-secondary)] transition-colors pointer-events-none">
                                        <Upload className="mr-1 h-3.5 w-3.5" /> {company?.logo ? 'Change Logo' : 'Upload Logo'}
                                    </span>
                                </label>
                                {company?.logo && (
                                    <button
                                        ...
                                        className="inline-flex items-center rounded-lg border border-[var(--border)] px-3 py-1.5 text-sm font-medium text-[var(--text-muted)] hover:bg-[var(--bg-secondary)] hover:text-[var(--error)] transition-colors disabled:opacity-50"
                                    >
                ...
                <div className="grid gap-6 lg:grid-cols-4">
                    ...
                    <div className="lg:col-span-3 space-y-6">
                        <Card header={<h2 className="text-lg font-semibold text-[var(--text)]">{SECTION_HEADERS[activeSection]}</h2>}>
                            {renderSection()}
                        </Card>
