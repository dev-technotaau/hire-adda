...
import {
    ...
    Globe, Code, Award, BookOpen, Calendar, Building2, Clock,
    CheckCircle, ExternalLink, Languages, Trophy, Heart,
    FolderKanban, Palette, FileText, Bookmark, Users,
    Download, Video, Car, Shield, Flag, Star, UserCheck,
    Linkedin, Github, Accessibility, MessageCircle, Activity, Pencil,
...
export default function EmployerCandidateDetailPage() {
    ...
    const handleResumeDownload = useCallback(async (applicationId?: string) => {
        if (!id) return;
        try {
            ...
            <div className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <Button
                            size="sm"
                            onClick={() => { setJobPickerAction('shortlist'); setJobPickerOpen(true); }}
                        >
                        ...
                        <Button
                            size="sm"
                            variant="outline"
                            onClick={() => { setJobPickerAction('select'); setJobPickerOpen(true); }}
                        >
                        ...
                        <div className="relative">
                            ...
                            >
                                <Phone className="mr-1.5 h-4 w-4" /> Contact <ChevronDown className="ml-1 h-3 w-3" />
                            </Button>
                            {contactOpen && (
                                <>
                                    <div className="fixed inset-0 z-40" onClick={() => setContactOpen(false)} />
                                    <div className="absolute right-0 top-full z-50 mt-1 w-52 rounded-lg border border-[var(--border)] bg-[var(--bg)] p-1 shadow-lg">
                                        {profile?.user?.email && (
                                            <a
                                        ...
                                        {(profile?.user?.whatsappNumber || profile?.user?.mobileNumber || profile?.phone) && (
                                            <a
                        ...
                        {profile?.resume && (
                            <Button variant="ghost" size="sm" title="Download Resume" onClick={() => handleResumeDownload()}>
                    ...
                    <>
                            ...
                            <div className="flex flex-col gap-6 sm:flex-row">
                                ...
                                <div className="flex-1 space-y-3">
                                    ...
                                    <div className="flex flex-wrap gap-3 text-sm text-[var(--text-secondary)]">
                                        ...
                                        {profile.experienceYears > 0 && (
                                            <span className="flex items-center gap-1">
                                                ...
                                                {profile.totalExperienceMonths != null && profile.totalExperienceMonths % 12 !== 0 && (
                                                    <span className="text-xs text-[var(--text-muted)]">({profile.totalExperienceMonths} months)</span>
                                                )}
                                            </span>
                                        )}
                                    ...
                                    <div className="flex flex-wrap gap-4 text-sm text-[var(--text-muted)]">
                                        {profile.user?.email && (
                                            <a href={`mailto:${profile.user.email}`} className="flex items-center gap-1 hover:text-primary">
                        ...
                        <div className="grid gap-6 lg:grid-cols-3">
                            ...
                            <div className="space-y-6 lg:col-span-2">
                                ...
                                {/* Education */}
                                {(profile.education && profile.education.length > 0) || profile.highestEducationLevel || profile.highestDegree ? (
                                    <Card>
                                        <div className="mb-4 flex items-center gap-2">
                                            ...
                                            {(profile.education || []).map((edu, i) => (
                                                <div key={i} className="flex gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        {edu.description && (
                                                            <p className="mt-1 text-sm text-[var(--text-secondary)]">{edu.description}</p>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        <div className="grid gap-3 sm:grid-cols-2">
                                            {profile.certifications.map((cert, i) => (
                                                <div key={i} className="rounded-lg border border-[var(--border)] p-3">
                                                    ...
                                                    {cert.credentialId && (
                                                        <p className="text-xs text-[var(--text-muted)]">ID: {cert.credentialId}</p>
                                                    )}
                                                    {cert.url && (
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            {profile.patents?.map((pat, i) => (
                                                <div key={`pat-${i}`} className="rounded-lg border border-[var(--border)] p-3">
                                                    ...
                                                    {pat.description && <p className="mt-1 text-sm text-[var(--text-secondary)]">{pat.description}</p>}
                                                    {pat.url && (
                                                        <a href={pat.url} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs text-primary hover:underline">
                                                            <ExternalLink className="h-3 w-3" /> View
                                                        </a>
                                                    )}
                                                </div>
                                                ...
                                                <div className="grid gap-2 sm:grid-cols-2">
                                                    {profile.testScores.map((test, i) => (
                                                        <div key={i} className="rounded border border-[var(--border)] p-2.5">
                                                            ...
                                                            {test.description && <p className="text-xs text-[var(--text-secondary)]">{test.description}</p>}
                                                        </div>
                                                    ))}
                                                </div>
                                            </div>
                                        ...
                                        <div className="grid gap-3 sm:grid-cols-2">
                                            {profile.professionalMemberships.map((mem, i) => (
                                                <div key={i} className="rounded-lg border border-[var(--border)] p-3">
                                                    ...
                                                    {mem.membershipId && <p className="text-xs text-[var(--text-muted)]">ID: {mem.membershipId}</p>}
                                                    {mem.description && <p className="mt-1 text-sm text-[var(--text-secondary)]">{mem.description}</p>}
                                                </div>
                                            ))}
                                        </div>
                                    </Card>
                            ...
                            <div className="space-y-6">
                                ...
                                {profile.skillsWithProficiency && profile.skillsWithProficiency.length > 0 && (
                                    <Card>
                                        <h2 className="mb-3 text-base font-semibold text-[var(--text)]">Skill Proficiency</h2>
                                        <div className="space-y-2">
                                            {profile.skillsWithProficiency.map((s, i) => (
                                                <div key={i} className="flex items-center justify-between text-sm">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <Badge
                                                            variant={s.proficiency === 'EXPERT' ? 'success' : s.proficiency === 'ADVANCED' ? 'info' : 'neutral'}
                                                            size="sm"
                                                        >
                                                            {s.proficiency}
                                                        </Badge>
                                                        {s.yearsOfExperience != null && (
                                                            <span className="text-xs text-[var(--text-muted)]">{s.yearsOfExperience}yr</span>
                                                        )}
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        <div className="space-y-2">
                                            {profile.itSkills.map((it, i) => (
                                                <div key={i} className="flex items-center justify-between text-sm">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        {it.proficiency && (
                                                            <Badge variant="neutral" size="sm">{it.proficiency}</Badge>
                                                        )}
                                ...
                                <Card>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {profile.preferredIndustries.length > 0 && (
                                            <div>
                                                <p className="text-xs text-[var(--text-muted)] mb-1">Preferred Industries</p>
                                                <div className="flex flex-wrap gap-1.5">
                                        ...
                                        {profile.preferredRoleCategories.length > 0 && (
                                            <div>
                                                <p className="text-xs text-[var(--text-muted)] mb-1">Preferred Roles</p>
                                                <div className="flex flex-wrap gap-1.5">
                                ...
                                {/* Additional Info */}
                                {(profile.visaStatus || profile.workPermitStatus || profile.hasDrivingLicense || profile.drivingLicenseType || profile.ownVehicle || profile.isPhysicallyChallenged) && (
                                    <Card>
                                        ...
                                        <div className="space-y-2 text-sm">
                                            ...
                                            {(profile.drivingLicenseType && profile.drivingLicenseType !== 'NONE') ? (
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="text-[var(--text)]">{DRIVING_LICENSE_TYPE_LABELS[profile.drivingLicenseType]}</span>
                                                </div>
                                            ...
                                            {profile.isPhysicallyChallenged && (
                                                <div className="flex justify-between">
                                                    ...
                                                    <span className="text-[var(--text)]">
                                                        {profile.disabilityType ? DISABILITY_TYPE_LABELS[profile.disabilityType] || profile.disabilityType : 'Yes'}
                                            ...
                                            {profile.languageProficiency.map((lang, i) => (
                                                <div key={i} className="flex items-center justify-between text-sm">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {lang.readWrite && <span className="text-xs text-[var(--text-muted)]">{lang.readWrite.replace(/_/g, ', ')}</span>}
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                ...
                                {(profile.resume || profile.videoResumeUrl) && (
                                    <Card>
                                        ...
                                        <div className="space-y-2">
                                            {profile.resume && (
                                                <button type="button" onClick={() => handleResumeDownload()} className="flex w-full items-center gap-2 rounded-lg border border-[var(--border)] p-3 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-secondary)]">
                                                    <FileText className="h-4 w-4 text-primary" />
                                                    <span className="flex-1 text-left">{profile.resumeOriginalName || 'Resume'}</span>
                                                    <Download className="h-4 w-4" />
                                                </button>
                                            )}
                                            {profile.videoResumeUrl && (
                                                <a href={profile.videoResumeUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 rounded-lg border border-[var(--border)] p-3 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-secondary)]">
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="min-w-0">
                                                    <p className="font-medium text-[var(--text)]">{app.job?.title || 'Unknown Job'}</p>
