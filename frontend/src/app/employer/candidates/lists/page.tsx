...
import {
    FolderOpen, Plus, MoreVertical, Edit2, Trash2,
    User, MapPin, Briefcase, Mail, Phone,
    X, Loader2, Search, StickyNote, Building2,
} from 'lucide-react';
...
import { candidateListService, type CandidateList, type CandidateListMember } from '@/services/candidate-list.service';
...
export default function CandidateListsPage() {
    ...
    const [memberToRemove, setMemberToRemove] = useState<{ listId: string; candidateId: string } | null>(null);
    const [notesDialogOpen, setNotesDialogOpen] = useState(false);
    const [notesEdit, setNotesEdit] = useState<{ listId: string; candidateId: string; notes: string }>({ listId: '', candidateId: '', notes: '' });
    ...
    const selectedList = lists.find((l) => l.id === selectedListId) || (lists.length > 0 ? lists[0] : null);
    ...
    const updateListMutation = useMutation({
        mutationFn: ({ listId, data }: { listId: string; data: { name?: string; description?: string | null; color?: string; icon?: string | null } }) =>
    ...
    const updateNotesMutation = useMutation({
        mutationFn: ({ listId, candidateId, notes }: { listId: string; candidateId: string; notes: string }) =>
                                            ...
                                            {!list.isDefault && (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <button
                                                        ...
                                                        className="rounded p-1 hover:bg-[var(--bg-tertiary)] text-[var(--error)]"
                                                        title="Delete list"
                                                    >
                        ...
                        {selectedList ? (
                            <Card>
                                <div className="mb-4 flex items-start justify-between">
                                    ...
                                    <Badge variant="info">
                                        {selectedList._count?.members || 0} candidates
                                    </Badge>
                                </div>
                                        ...
                                        <div className="space-y-3">
                                            {members.map((member) => {
                                                ...
                                                return (
                                                    <div
                                                        key={member.id}
                                                        className="rounded-lg border border-[var(--border)] bg-white p-4 transition-colors hover:border-primary/20"
                                                    >
                                                            ...
                                                            <div className="flex gap-3 min-w-0 flex-1">
                                                                ...
                                                                <div className="min-w-0 flex-1">
                                                                    ...
                                                                    <p className="mt-2 text-xs text-[var(--text-muted)]">
                                                                        ...
                                                                        {member.addedBy && ` by ${member.addedBy.firstName} ${member.addedBy.lastName}`}
                                                                    </p>
                                                                </div>
                                                            </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                        <div className="w-full max-w-md rounded-xl bg-[var(--bg)] p-6 shadow-xl">
                            ...
                            <textarea
                                ...
                                rows={4}
                                className="w-full rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-sm text-[var(--text)] placeholder:text-[var(--text-muted)] focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                            />
...
function EditListDialog({
    ...
    list: CandidateList;
    onClose: () => void;
    onSubmit: (data: { name?: string; description?: string | null; color?: string; icon?: string | null }) => void;
    isLoading: boolean;
}) {
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            value={name}
                            onChange={(e) => setName(e.target.value)}
                            className="w-full rounded-lg border border-[var(--border)] bg-white px-3 py-2 text-sm text-[var(--text)] focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
                        />
                    </div>
