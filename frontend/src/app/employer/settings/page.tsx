    ...
    <Card variant="bordered">
      <div className="mb-6 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--text-secondary)]">Your basic account details</p>
        </div>
      </div>
    ...
    <Card variant="bordered">
      ...
      {step === 'form' ? (
        <form onSubmit={handleInitiate} className="max-w-md space-y-4">
          ...
          />
          <div className="pt-2">
            <Button type="submit" isLoading={isLoading} tooltip="Initiate email change process">
              Change Email
            </Button>
          </div>
        </form>
    ...
    <Card variant="bordered">
      ...
      {step === 'change' && (
        <form onSubmit={handleInitiateChange} className="max-w-md space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <Button type="submit" isLoading={isLoading} tooltip="Submit new phone number">
              Change Number
            </Button>
            <Button variant="outline" onClick={handleCancel} tooltip="Cancel phone number change">
              Cancel
            </Button>
          </div>
        </form>
        ...
        <div className="max-w-md space-y-4">
          ...
          <div className="flex gap-3 pt-1">
            <Button
              ...
              disabled={otp.length !== otpConfig.LENGTH}
              tooltip="Verify phone number with OTP"
            >
              Verify
            </Button>
            <Button variant="outline" onClick={handleCancel} tooltip="Cancel phone verification">
              Cancel
            </Button>
          </div>
        </div>
      ...
      {isVerified && step === 'idle' ? (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Tooltip content="Remove WhatsApp verification from your account">
              <button
                type="button"
        ...
        <div className="max-w-md space-y-4">
          ...
          <div className="text-center">
            <p className="text-sm text-[var(--text-muted)]">
              ...
              {resendTimer > 0 ? (
                ...
                <Tooltip content="Resend OTP to your WhatsApp">
                  <button
                    type="button"
                    onClick={handleResend}
                    disabled={isResending}
          ...
          <div className="flex gap-3 pt-1">
            <Button
              ...
              onClick={handleVerify}
              disabled={otp.length !== otpConfig.LENGTH}
              tooltip="Verify WhatsApp with OTP"
            >
              Verify
            </Button>
            <Button variant="outline" onClick={resetToIdle} tooltip="Cancel WhatsApp verification">
              Cancel
            </Button>
          </div>
        </div>
    ...
    <Card variant="bordered">
      ...
      {step === 'form' ? (
        <form onSubmit={handleInitiate} className="max-w-md space-y-4">
          ...
          />
          <div className="pt-2">
            <Button type="submit" isLoading={isLoading} tooltip="Initiate password change">
              Update Password
            </Button>
          </div>
        </form>
      ...
      <Card variant="bordered" className="border-opacity-30 border-[var(--error)]">
        ...
        <div className="border-opacity-30 flex items-center justify-between rounded-lg border border-[var(--error)] p-4">
          <div>
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              Permanently delete your account, job postings, and all associated data. This action
              cannot be undone.
            </p>
          </div>
          <Button
      ...
      >
        <div className="space-y-4">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm text-[var(--text-secondary)]">
              This action is <strong className="text-[var(--error)]">permanent</strong> and cannot
              be undone. All your data, including your company profile, job postings, and
          ...
          newBackupCodes.length > 0 ? (
            <div className="flex justify-end">
              <Button
                ...
                tooltip="Close and save your backup codes"
              >
                Done
              </Button>
            </div>
              ...
              <div key={wh.id} className="rounded-lg border border-[var(--border)]">
                <div className="flex items-center justify-between p-3">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <p className="truncate text-sm font-medium text-[var(--text)]">{wh.url}</p>
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ${wh.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}
                      >
                    ...
                    <p className="mt-0.5 text-xs text-[var(--text-muted)]">
                      ...
                      {wh.lastTriggeredAt &&
                        ` \u00B7 Last triggered ${formatRelativeDate(wh.lastTriggeredAt)}`}
                    </p>
                  </div>
                  <div className="ml-2 flex items-center gap-1">
                    <Button
                      ...
                      tooltip={wh.isActive ? 'Disable webhook' : 'Enable webhook'}
                    >
                      {wh.isActive ? (
      ...
      {copiedSecret && (
        ...
        >
          ...
          <div className="flex items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--bg-secondary)] p-3">
            <code className="flex-1 font-mono text-xs break-all text-[var(--text)]">
              {copiedSecret}
            </code>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => {
                navigator.clipboard.writeText(copiedSecret);
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              tooltip="Create webhook endpoint"
            >
              Create Webhook
            </Button>
          </div>
          ...
          {deliveries.slice(0, 10).map((d) => (
            <tr key={d.id}>
              ...
              <td className="px-3 py-2">
                <span
                  className={`inline-flex items-center rounded-full px-1.5 py-0.5 text-xs font-medium ${d.success ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
                >
...
function NotificationsTab() {
  ...
  useEffect(() => {
    const p = (companyData?.data as unknown as Record<string, unknown>)?.notificationPreferences as
    ...
    <div className="space-y-6">
      ...
      <Card variant="bordered">
        ...
        <Button
          ...
          tooltip="Export your data as JSON"
        >
