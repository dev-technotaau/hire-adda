...
import {
    Building2, Globe, Briefcase, Users, Award,
    MapPin, Mail, Phone, Plus, Trash2,
    Sparkles, Target, Heart, Shield, Zap,
    Linkedin, Twitter, Facebook, Instagram, Youtube,
    Calendar, DollarSign, ExternalLink,
    Gem, BookOpen, HandHeart, TrendingUp, UserCircle,
    Camera, Briefcase as BriefcaseBusiness,
} from 'lucide-react';
...
import {
    INDUSTRY_SUGGESTIONS, LOCATION_SUGGESTIONS, INDIAN_STATES,
    BENEFIT_SUGGESTIONS, SKILL_SUGGESTIONS, REVENUE_RANGE_OPTIONS,
    SUB_INDUSTRY_SUGGESTIONS, CORE_VALUE_SUGGESTIONS, INVESTOR_SUGGESTIONS,
    PRODUCT_SERVICE_SUGGESTIONS, ERG_SUGGESTIONS,
} from '@/constants/suggestions';
import { useAuth } from '@/hooks/use-auth';
import type { UpdateCompanyRequest, LeadershipEntry, EmployeeTestimonialEntry } from '@/types/employer';
...
export default function EmployerOnboardingPage() {
    ...
    const handleNext = useCallback(async () => {
        ...
        if (isLastStep) {
            ...
            const payload: UpdateCompanyRequest = {
                ...
                employeeResourceGroups: data.employeeResourceGroups.length > 0 ? data.employeeResourceGroups : undefined,
                ...
                workplacePolicies: Object.keys(data.workplacePolicies).length > 0 ? data.workplacePolicies : undefined,
                awardsRecognitions: data.awardsRecognitions.length > 0 ? data.awardsRecognitions : undefined,
                ...
                employeeTestimonials: data.employeeTestimonials.length > 0 ? data.employeeTestimonials : undefined,
    ...
    // --- Tag helpers --------------------------------------------------------
    const addBenefit = useCallback((value: string) => {
    ...
    const removeBenefit = useCallback((value: string) => {
        updateData({ benefits: data.benefits.filter((b) => b !== value) });
    }, [data.benefits, updateData]);

    const addTech = useCallback((value: string) => {
        const trimmed = value.trim();
    ...
    const removeTech = useCallback((value: string) => {
        updateData({ techStack: data.techStack.filter((t) => t !== value) });
    }, [data.techStack, updateData]);

    const addLocation = useCallback((value: string) => {
        const trimmed = value.trim();
    ...
    const removeLocation = useCallback((value: string) => {
        updateData({ locations: data.locations.filter((l) => l !== value) });
    }, [data.locations, updateData]);

    const addSpecialty = useCallback((value: string) => {
        const trimmed = value.trim();
    ...
    const addCoreValue = useCallback((value: string) => {
        const trimmed = value.trim();
    ...
    const removeCoreValue = useCallback((value: string) => {
        updateData({ coreValues: data.coreValues.filter((v) => v !== value) });
    }, [data.coreValues, updateData]);

    const addInvestor = useCallback((value: string) => {
        const trimmed = value.trim();
    ...
    const removeInvestor = useCallback((value: string) => {
        updateData({ investors: data.investors.filter((i) => i !== value) });
    }, [data.investors, updateData]);

    const addProduct = useCallback((value: string) => {
        const trimmed = value.trim();
    ...
    const removeProduct = useCallback((value: string) => {
        updateData({ productsServices: data.productsServices.filter((p) => p !== value) });
    }, [data.productsServices, updateData]);

    const addErg = useCallback((value: string) => {
        const trimmed = value.trim();
    ...
    const removeErg = useCallback((value: string) => {
        updateData({ employeeResourceGroups: data.employeeResourceGroups.filter((e) => e !== value) });
    }, [data.employeeResourceGroups, updateData]);
    ...
    const updateAward = useCallback((index: number, field: string, value: string | number | undefined) => {
    ...
    const updateLeadership = useCallback((index: number, field: string, value: string) => {
    ...
    // --- Social link helper -------------------------------------------------
    const updateSocial = useCallback((platform: keyof EmployerOnboardingData['socialLinks'], value: string) => {
        updateData({
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <p className="mb-3 text-xs text-[var(--text-muted)]">
                    Areas your company specializes in
                </p>
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">This is optional but highly recommended</p>
                </div>
            </div>
                            ...
                            <button
                                ...
                                className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${
                                    isAdded
                                        ...
                                        : 'border-[var(--border)] bg-white text-[var(--text-secondary)] hover:border-primary hover:text-primary'
                                }`}
                            >
                                {isAdded ? '+ ' : '+ '}{benefit}
                            </button>
            ...
            <div>
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-[var(--text-muted)]">Showcase your company&apos;s key leaders</p>
                    </div>
                </div>
                    ...
                    >
                        <div className="mb-3 flex items-center justify-between">
                            ...
                            <button
                                ...
                                className="rounded-md p-1 text-[var(--text-muted)] transition-colors hover:bg-white hover:text-error"
                            >
            ...
            <div>
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-[var(--text-muted)]">Share what your employees say about working here</p>
                    </div>
                </div>
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">Share your company&apos;s funding journey (optional)</p>
                </div>
            </div>
    ...
    const renderLegal = () => (
        <div className="space-y-6">
            <p className="text-sm text-[var(--text-muted)]">
                These details are optional and kept private. They help verify your company and are
                not shown publicly.
            </p>
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">Help candidates understand what to expect</p>
                </div>
            </div>
        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    ...
                    <p className="text-xs text-[var(--text-muted)]">Share your company&apos;s social impact initiatives</p>
                </div>
            </div>
    ...
    const renderReview = () => {
        const sections: Array<{
            ...
            },
            {
                ...
                icon: Target,
                items: [
                    { label: 'Mission', value: data.missionStatement ? `${data.missionStatement.slice(0, 80)}...` : undefined },
                    { label: 'Vision', value: data.visionStatement ? `${data.visionStatement.slice(0, 80)}...` : undefined },
                    ...
                    { label: 'Core Values', value: data.coreValues.length > 0 ? data.coreValues.join(', ') : undefined },
                    { label: 'ERGs', value: data.employeeResourceGroups.length > 0 ? `${data.employeeResourceGroups.length} groups` : undefined },
                ],
            },
            {
                ...
                icon: Heart,
                items: [
                    { label: 'Benefits', value: data.benefits.length > 0 ? `${data.benefits.length} added` : undefined },
                ],
            },
            {
                title: 'Tech & Policies',
                ...
                icon: Zap,
                items: [
                    { label: 'Tech Stack', value: data.techStack.length > 0 ? `${data.techStack.length} technologies` : undefined },
                    { label: 'Products & Services', value: data.productsServices.length > 0 ? `${data.productsServices.length} items` : undefined },
                    {
                        label: 'Policies',
                        value: Object.values(data.workplacePolicies).filter(Boolean).length > 0
            ...
            },
            {
                ...
                icon: UserCircle,
                items: [
                    { label: 'Leadership Team', value: data.leadershipTeam.length > 0 ? `${data.leadershipTeam.length} leaders` : undefined },
                    { label: 'Testimonials', value: data.employeeTestimonials.length > 0 ? `${data.employeeTestimonials.length} testimonials` : undefined },
                ],
            },
            {
                title: 'Awards',
                ...
                items: [
                    {
                        label: 'Awards',
                        value: data.awardsRecognitions.length > 0
            ...
            },
            {
                ...
                icon: TrendingUp,
                items: [
                    { label: 'Funding Stage', value: data.fundingStage ? FUNDING_STAGE_LABELS[data.fundingStage] : undefined },
                    { label: 'Total Funding', value: data.totalFundingRaised },
                    { label: 'Investors', value: data.investors.length > 0 ? data.investors.join(', ') : undefined },
                ],
            },
            {
                title: 'Contact Info',
